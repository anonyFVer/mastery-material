...
public class GlideTest {
    ...
    public void setUp() throws Exception {
        ...
        Glide.setup(new GlideBuilder(Robolectric.application).setMemoryCache(mock(MemoryCache.class)).setDiskCache(mock(DiskCache.class)).setResizeService(service).setDiskCacheService(service));
        ...
        when(mockUrlLoaderFactory.build(any(Context.class), any(GenericLoaderFactory.class))).thenReturn(mockUrlLoader);
    ...
    public void testGenericLoader() throws Exception {
        ...
        when(modelLoader.getResourceFetcher(eq(glideUrl), anyInt(), anyInt())).thenReturn(dataFetcher);
        ...
        requestManager.using(modelLoader, File.class).load(glideUrl).as(File.class).decoder(sourceDecoder).cacheDecoder(cacheDecoder).encoder(encoder).sourceEncoder(sourceEncoder).into(target);
    ...
    public void testTranscodeOption() {
        ...
        requestManager.load(uri).asBitmap().transcode(transcoder, byte[].class).into(target);
    ...
    private void runTestStringDefaultLoader(String string) {
        requestManager.load(string).listener(new RequestListener<String, GlideDrawable>() {
    ...
    public void testUnregisteredModelWithGivenLoaderDoesNotThrow() {
        ...
        requestManager.using(mockLoader).load(unregistered).into(target);
    }
    ...
    public void testReceivesGif() throws IOException {
        ...
        requestManager.load(fakeUri).asGif().into(target);
    ...
    public void testReceivesGifBytes() throws IOException {
        ...
        InputStream testGifData = openResource("test.gif");
        mockUri(Uri.parse(fakeUri), testGifData);
        requestManager.load(fakeUri).asGif().toBytes().into(target);
        verify(target).onResourceReady(any(byte[].class), any(GlideAnimation.class));
    }
    ...
    public void testReceivesBitmapBytes() {
        String fakeUri = "content://fake";
        mockUri(fakeUri);
        requestManager.load(fakeUri).asBitmap().toBytes().into(target);
        verify(target).onResourceReady(any(byte[].class), any(GlideAnimation.class));
    }
    ...
    public void testReceivesTranscodedData() {
        ...
        requestManager.load(fakeUri).asBitmap().transcode(new ResourceTranscoder<Bitmap, Bitmap>() {
    ...
    public void testNullModelDoesNotThrow() {
        ...
        requestManager.load(nullString).placeholder(drawable).into(target);
    ...
    public void testNullModelPrefersErrorDrawable() {
        ...
        requestManager.load(nullString).placeholder(placeholder).error(error).into(target);
    ...
    public void testNullModelWithModelLoaderDoesNotThrow() {
        ...
        requestManager.using(modelLoader).load(nullString).placeholder(drawable).into(target);
        verify(target).onLoadFailed(any(Exception.class), eq(drawable));
    }
    ...
    public void testClone() throws IOException {
        ...
        DrawableRequestBuilder<String> firstRequest = requestManager.from(String.class).transcoder(firstTranscoder).override(100, 100);
    ...
    private <T> void registerMockStreamModelLoader(final Class<T> modelClass) {
        ...
        when(modelLoaderFactory.build(any(Context.class), any(GenericLoaderFactory.class))).thenReturn(modelLoader);
    ...
    private <T> StreamModelLoader<T> mockStreamModelLoader(final Class<T> modelClass) {
        ...
        when(modelLoader.getResourceFetcher(any(modelClass), anyInt(), anyInt())).thenReturn(fetcher);
        return modelLoader;
    }
    ...
    public static class ShadowFileDescriptorContentResolver {
        ...
        private static final Map<Uri, AssetFileDescriptor> URI_TO_FILE_DESCRIPTOR = new HashMap<Uri, AssetFileDescriptor>();
