...
@Config(manifest = Config.NONE, emulateSdk = 18, shadows = { GlideTest.ShadowFileDescriptorContentResolver.class, GlideTest.ShadowMediaMetadataRetriever.class, GlideShadowLooper.class, GlideTest.MutableShadowBitmap.class })
public class GlideTest {
    ...
    public void testCanSetMemoryCategory() {
        ...
        Glide glide = new GlideBuilder(getContext()).setMemoryCache(memoryCache).setBitmapPool(bitmapPool).createGlide();
    ...
    public void testClearMemory() {
        BitmapPool bitmapPool = mock(BitmapPool.class);
        MemoryCache memoryCache = mock(MemoryCache.class);
        Glide glide = new GlideBuilder(getContext()).setBitmapPool(bitmapPool).setMemoryCache(memoryCache).createGlide();
        glide.clearMemory();
    ...
    private void runTestStringDefaultLoader(String string) {
        requestManager.asDrawable().load(string).listener(new RequestListener<Drawable>() {
            ...
            @Override
            public boolean onLoadFailed(Object model, Target target, boolean isFirstResource) {
    ...
    public void testReceivesGifBytes() throws IOException {
        ...
        InputStream testGifData = openResource("test.gif");
        mockUri(Uri.parse(fakeUri), testGifData);
        requestManager.as(byte[].class).apply(decodeTypeOf(GifDrawable.class)).load(fakeUri).into(target);
        verify(target).onResourceReady(any(byte[].class), any(Transition.class));
    }
    ...
    public void testReceivesThumbnails() {
        ...
        requestManager.asDrawable().load(full).thumbnail(requestManager.asDrawable().load(thumb)).into(target);
        verify(target, times(2)).onResourceReady(any(Drawable.class), any(Transition.class));
    }

    @Test
    public void testReceivesRecursiveThumbnails() {
        requestManager.asDrawable().load(mockUri("content://first")).thumbnail(requestManager.asDrawable().load(mockUri("content://second")).thumbnail(requestManager.asDrawable().load(mockUri("content://third")).thumbnail(requestManager.asDrawable().load(mockUri("content://fourth"))))).into(target);
    ...
    public void testReceivesRecursiveThumbnailWithPercentage() {
        requestManager.asDrawable().load(mockUri("content://first")).thumbnail(requestManager.asDrawable().load(mockUri("content://second")).thumbnail(0.5f)).into(target);
    ...
    public void testNullModelPrefersErrorDrawable() {
        ...
        requestManager.asDrawable().load(nullString).apply(placeholderOf(placeholder).error(error)).into(target);
    ...
    public void testClone() throws IOException {
        ...
        RequestBuilder<Drawable> firstRequest = Glide.with(getContext()).asDrawable().load(mockUri("content://first"));
    ...
    private <T, Z> void registerFailFactory(Class<T> failModel, Class<Z> failResource) throws Exception {
        ...
        doAnswer(new Util.CallDataReady(null)).when(failFetcher).loadData(any(Priority.class), any(DataFetcher.DataCallback.class));
        when(failFetcher.getDataClass()).thenReturn(failResource);
        ModelLoader<T, Z> failLoader = mock(ModelLoader.class);
        when(failLoader.buildLoadData(any(failModel), anyInt(), anyInt(), any(Options.class))).thenReturn(new ModelLoader.LoadData<>(mock(Key.class), failFetcher));
    ...
    private <T> void registerMockStreamModelLoader(final Class<T> modelClass) {
        ...
        when(modelLoaderFactory.build(any(Context.class), any(MultiModelLoaderFactory.class))).thenReturn(modelLoader);
        Glide.get(RuntimeEnvironment.application).getRegistry().prepend(modelClass, InputStream.class, modelLoaderFactory);
    }
    ...
    private <T> ModelLoader<T, InputStream> mockStreamModelLoader(final Class<T> modelClass) {
        ...
        try {
            doAnswer(new Util.CallDataReady(new ByteArrayInputStream(new byte[0]))).when(fetcher).loadData(any(Priority.class), any(DataFetcher.DataCallback.class));
        } catch (Exception e) {
        }
        when(fetcher.getDataClass()).thenReturn(InputStream.class);
        when(modelLoader.buildLoadData(any(modelClass), anyInt(), anyInt(), any(Options.class))).thenReturn(new ModelLoader.LoadData<>(mock(Key.class), fetcher));
    ...
    public static class SetupModule implements GlideModule {
        ...
        public void applyOptions(Context context, GlideBuilder builder) {
            ...
            builder.setMemoryCache(mock(MemoryCache.class)).setDiskCache(diskCacheFactory).setResizeService(service).setDiskCacheService(service);
        }

        @Override
        public void registerComponents(Context context, Registry registry) {
            registerMockModelLoader(GlideUrl.class, InputStream.class, new ByteArrayInputStream(new byte[0]), registry);
            registerMockModelLoader(File.class, InputStream.class, new ByteArrayInputStream(new byte[0]), registry);
            registerMockModelLoader(File.class, ParcelFileDescriptor.class, mock(ParcelFileDescriptor.class), registry);
            registerMockModelLoader(File.class, ByteBuffer.class, ByteBuffer.allocate(10), registry);
        }

        private static <X, Y> void registerMockModelLoader(Class<X> modelClass, Class<Y> dataClass, Y loadedData, Registry registry) {
            DataFetcher<Y> mockStreamFetcher = mock(DataFetcher.class);
            when(mockStreamFetcher.getDataClass()).thenReturn(dataClass);
            try {
                doAnswer(new Util.CallDataReady<>(loadedData)).when(mockStreamFetcher).loadData(any(Priority.class), any(DataFetcher.DataCallback.class));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            ModelLoader<X, Y> mockUrlLoader = mock(ModelLoader.class);
            when(mockUrlLoader.buildLoadData(any(modelClass), anyInt(), anyInt(), any(Options.class))).thenReturn(new ModelLoader.LoadData<>(mock(Key.class), mockStreamFetcher));
            ...
            when(mockUrlLoaderFactory.build(any(Context.class), any(MultiModelLoaderFactory.class))).thenReturn(mockUrlLoader);
