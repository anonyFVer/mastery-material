...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback, Runnable, Prioritized {
    ...
    public DecodeJob(RequestContext<?, R> requestContext, EngineKey loadKey, int width, int height, DiskCacheProvider diskCacheProvider, Callback<R> callback) {
    ...
    private enum Stage {
        ...
        INITIALIZE, RESOURCE_CACHE, DATA_CACHE, SOURCE
    }
    ...
    @Override
    public void onDataFetcherReady(Key sourceKey, Object data, DataFetcher<?> fetcher, DataSource dataSource) {
    ...
    private <Data> Resource<R> decodeFromData(DataFetcher<?> fetcher, Data data, DataSource dataSource) {
        try {
            if (data == null) {
                return null;
            }
    ...
    private void logWithTimeAndKey(String message, long startTime, String extraArgs) {
        Logs.log(Log.VERBOSE, message + " in " + LogTime.getElapsedMillis(startTime) + ", load key: " + loadKey + (extraArgs != null ? ", " + extraArgs : "") + ", thread: " + Thread.currentThread().getName());
    }
