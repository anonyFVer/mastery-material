...
public class BaseGlideUrlLoaderTest {
    ...
    @Mock
    ModelLoader<GlideUrl, InputStream> wrapped;
    ...
    public void testDoesNotInteractWithModelCacheIfNull() {
        ...
        int width = 456;
        int height = 789;
        when(wrapped.buildLoadData(any(GlideUrl.class), eq(width), eq(height), eq(options))).thenReturn(new ModelLoader.LoadData<>(mock(Key.class), fetcher));
