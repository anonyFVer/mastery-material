...
public class RecoveryTests extends ESIndexLevelReplicationTestCase {
    ...
    public void testRecoveryWithOutOfOrderDelete() throws Exception {
        try (ReplicationGroup shards = createGroup(1)) {
            ...
            getTranslog(orgReplica).rollGeneration();
            orgReplica.applyIndexOperationOnReplica(0, 1, VersionType.EXTERNAL, IndexRequest.UNSET_AUTO_GENERATED_TIMESTAMP, false, SourceToParse.source(indexName, "type", "id", new BytesArray("{}"), XContentType.JSON));
            orgReplica.applyIndexOperationOnReplica(3, 1, VersionType.EXTERNAL, IndexRequest.UNSET_AUTO_GENERATED_TIMESTAMP, false, SourceToParse.source(indexName, "type", "id-3", new BytesArray("{}"), XContentType.JSON));
            orgReplica.flush(new FlushRequest().force(true).waitIfOngoing(true));
            orgReplica.applyIndexOperationOnReplica(2, 1, VersionType.EXTERNAL, IndexRequest.UNSET_AUTO_GENERATED_TIMESTAMP, false, SourceToParse.source(indexName, "type", "id-2", new BytesArray("{}"), XContentType.JSON));
            orgReplica.updateGlobalCheckpointOnReplica(3L, "test");
            orgReplica.applyIndexOperationOnReplica(5, 1, VersionType.EXTERNAL, IndexRequest.UNSET_AUTO_GENERATED_TIMESTAMP, false, SourceToParse.source(indexName, "type", "id-5", new BytesArray("{}"), XContentType.JSON));
            ...
            if (randomBoolean()) {
                if (randomBoolean()) {
                    ...
                    builder.settings(Settings.builder().put(orgReplica.indexSettings().getSettings()).put(IndexSettings.INDEX_TRANSLOG_RETENTION_AGE_SETTING.getKey(), "-1").put(IndexSettings.INDEX_TRANSLOG_RETENTION_SIZE_SETTING.getKey(), "-1"));
