...
public class TransportDeleteAction extends TransportWriteAction<DeleteRequest, DeleteRequest, DeleteResponse> {
    ...
    protected void doExecute(Task task, final DeleteRequest request, final ActionListener<DeleteResponse> listener) {
        ...
        if (autoCreateIndex.shouldAutoCreate(request.index(), state)) {
            CreateIndexRequest createIndexRequest = new CreateIndexRequest().index(request.index()).cause("auto(delete api)").masterNodeTimeout(request.timeout());
    ...
    public static void resolveAndValidateRouting(final MetaData metaData, final String concreteIndex, DeleteRequest request) {
    ...
    protected WritePrimaryResult shardOperationOnPrimary(DeleteRequest request, IndexShard primary) throws Exception {
        ...
        if (result.hasFailure() == false) {
            ...
            response = new DeleteResponse(primary.shardId(), request.type(), request.id(), result.getSeqNo(), result.getVersion(), result.isFound());
        } else {
            response = null;
            replicaRequest = null;
        }
    ...
    public static Engine.DeleteResult executeDeleteRequestOnReplica(DeleteRequest request, IndexShard replica) {
        final Engine.Delete delete = replica.prepareDeleteOnReplica(request.type(), request.id(), request.getSeqNo(), request.primaryTerm(), request.version(), request.versionType());
