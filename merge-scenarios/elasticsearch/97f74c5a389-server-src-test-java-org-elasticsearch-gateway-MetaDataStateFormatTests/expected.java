...
public class MetaDataStateFormatTests extends ESTestCase {
    ...
    public void testFailWriteAndReadPreviousState() throws IOException {
        ...
        for (int i = 0; i < randomIntBetween(1, 5); i++) {
            format.failOnMethods(Format.FAIL_DELETE_TMP_FILE, Format.FAIL_CREATE_OUTPUT_FILE, Format.FAIL_WRITE_TO_OUTPUT_FILE, Format.FAIL_FSYNC_TMP_FILE, Format.FAIL_RENAME_TMP_FILE);
    ...
    private IndexMetaData.Builder indexBuilder(String index) throws IOException {
        return IndexMetaData.builder(index).settings(settings(Version.CURRENT).put(IndexMetaData.SETTING_NUMBER_OF_SHARDS, randomIntBetween(1, 10)).put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, randomIntBetween(0, 5)));
    }
