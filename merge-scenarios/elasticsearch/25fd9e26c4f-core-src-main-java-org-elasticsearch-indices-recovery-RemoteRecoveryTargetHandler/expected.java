...
public class RemoteRecoveryTargetHandler implements RecoveryTargetHandler {
    ...
    public FinalizeResponse finalizeRecovery() {
        return transportService.submitRequest(targetNode, PeerRecoveryTargetService.Actions.FINALIZE, new RecoveryFinalizeRecoveryRequest(recoveryId, shardId), TransportRequestOptions.builder().withTimeout(recoverySettings.internalActionLongTimeout()).build(), new FutureTransportResponseHandler<FinalizeResponse>() {
    ...
    public void indexTranslogOperations(List<Translog.Operation> operations, int totalTranslogOps) {
        ...
        transportService.submitRequest(targetNode, PeerRecoveryTargetService.Actions.TRANSLOG_OPS, translogOperationsRequest, translogOpsRequestOptions, EmptyTransportResponseHandler.INSTANCE_SAME).txGet();
    }
