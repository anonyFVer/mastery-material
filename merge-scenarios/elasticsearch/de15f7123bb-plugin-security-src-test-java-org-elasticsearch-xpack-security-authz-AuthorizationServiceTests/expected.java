...
public class AuthorizationServiceTests extends ESTestCase {
    ...
    public void testUserWithNoRolesCannotSql() {
        ...
        User user = new User("test user");
        mockEmptyMetaData();
        assertThrowsAuthorizationException(() -> authorize(createAuthentication(user), SqlQueryAction.NAME, request), SqlQueryAction.NAME, "test user");
    ...
    public void testUnknownRoleCausesDenial() {
        ...
        Tuple<String, TransportRequest> tuple = randomFrom(new Tuple<>(SearchAction.NAME, new SearchRequest()), new Tuple<>(IndicesExistsAction.NAME, new IndicesExistsRequest()), new Tuple<>(SqlQueryAction.NAME, new SqlQueryRequest()));
    ...
    public void testSearchAgainstEmptyCluster() {
        ...
        }
        {
            ...
            IndicesAccessControl.IndexAccessControl indexAccessControl = indicesAccessControl.getIndexPermissions(IndicesAndAliasesResolverField.NO_INDEX_PLACEHOLDER);
    ...
    public void testRunAsRequestWithoutLookedUpBy() {
        ...
        verify(auditTrail).runAsDenied(user, AuthenticateAction.NAME, request, new String[] { ReservedRolesStore.SUPERUSER_ROLE_DESCRIPTOR.getName() });
        verifyNoMoreInteractions(auditTrail);
    }
    ...
    public void testNonXPackUserCannotExecuteOperationAgainstSecurityIndex() {
        ...
        ClusterState state = mock(ClusterState.class);
        when(clusterService.state()).thenReturn(state);
        when(state.metaData()).thenReturn(MetaData.builder().put(new IndexMetaData.Builder(SECURITY_INDEX_NAME).settings(Settings.builder().put("index.version.created", Version.CURRENT).build()).numberOfShards(1).numberOfReplicas(0).build(), true).build());
        List<Tuple<String, TransportRequest>> requests = new ArrayList<>();
        requests.add(new Tuple<>(BulkAction.NAME + "[s]", new DeleteRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(UpdateAction.NAME, new UpdateRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(BulkAction.NAME + "[s]", new IndexRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(SearchAction.NAME, new SearchRequest(SECURITY_INDEX_NAME)));
        requests.add(new Tuple<>(TermVectorsAction.NAME, new TermVectorsRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(GetAction.NAME, new GetRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(TermVectorsAction.NAME, new TermVectorsRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(IndicesAliasesAction.NAME, new IndicesAliasesRequest().addAliasAction(AliasActions.add().alias("security_alias").index(SECURITY_INDEX_NAME))));
        requests.add(new Tuple<>(UpdateSettingsAction.NAME, new UpdateSettingsRequest().indices(SECURITY_INDEX_NAME)));
    ...
    public void testSuperusersCanExecuteOperationAgainstSecurityIndex() {
        ...
        when(state.metaData()).thenReturn(MetaData.builder().put(new IndexMetaData.Builder(SECURITY_INDEX_NAME).settings(Settings.builder().put("index.version.created", Version.CURRENT).build()).numberOfShards(1).numberOfReplicas(0).build(), true).build());
        List<Tuple<String, TransportRequest>> requests = new ArrayList<>();
        requests.add(new Tuple<>(DeleteAction.NAME, new DeleteRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(BulkAction.NAME + "[s]", createBulkShardRequest(SECURITY_INDEX_NAME, DeleteRequest::new)));
        requests.add(new Tuple<>(UpdateAction.NAME, new UpdateRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(IndexAction.NAME, new IndexRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(BulkAction.NAME + "[s]", createBulkShardRequest(SECURITY_INDEX_NAME, IndexRequest::new)));
        requests.add(new Tuple<>(SearchAction.NAME, new SearchRequest(SECURITY_INDEX_NAME)));
        requests.add(new Tuple<>(TermVectorsAction.NAME, new TermVectorsRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(GetAction.NAME, new GetRequest(SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(TermVectorsAction.NAME, new TermVectorsRequest(SECURITY_INDEX_NAME, "type", "id")));
        ...
        requests.add(new Tuple<>(ClusterHealthAction.NAME, new ClusterHealthRequest(SECURITY_INDEX_NAME, "foo", "bar")));
        for (Tuple<String, TransportRequest> requestTuple : requests) {
            String action = requestTuple.v1();
            TransportRequest request = requestTuple.v2();
    ...
    public void testSameUserPermission() {
        ...
        when(authenticatedBy.getType()).thenReturn(changePasswordRequest ? randomFrom(ReservedRealm.TYPE, NativeRealmSettings.TYPE) : randomAlphaOfLengthBetween(4, 12));
    ...
    public void testSameUserPermissionDoesNotAllowNonMatchingUsername() {
        ...
        when(lookedUpBy.getType()).thenReturn(changePasswordRequest ? randomFrom(ReservedRealm.TYPE, NativeRealmSettings.TYPE) : randomAlphaOfLengthBetween(4, 12));
        assertFalse(AuthorizationService.checkSameUserPermissions(action, request, authentication));
    ...
    public void testSameUserPermissionDoesNotAllowChangePasswordForOtherRealms() {
        ...
        when(authentication.getUser()).thenReturn(user);
        when(authentication.getAuthenticatedBy()).thenReturn(authenticatedBy);
        when(authenticatedBy.getType()).thenReturn(randomFrom(LdapRealmSettings.LDAP_TYPE, FileRealmSettings.TYPE, LdapRealmSettings.AD_TYPE, PkiRealmSettings.TYPE, randomAlphaOfLengthBetween(4, 12)));
        assertThat(request, instanceOf(UserRequest.class));
        assertFalse(AuthorizationService.checkSameUserPermissions(action, request, authentication));
    ...
    public void testSameUserPermissionDoesNotAllowChangePasswordForLookedUpByOtherRealms() {
        ...
        when(authentication.getAuthenticatedBy()).thenReturn(authenticatedBy);
        when(authentication.getLookedUpBy()).thenReturn(lookedUpBy);
        when(lookedUpBy.getType()).thenReturn(randomFrom(LdapRealmSettings.LDAP_TYPE, FileRealmSettings.TYPE, LdapRealmSettings.AD_TYPE, PkiRealmSettings.TYPE, randomAlphaOfLengthBetween(4, 12)));
        assertThat(request, instanceOf(UserRequest.class));
        assertFalse(AuthorizationService.checkSameUserPermissions(action, request, authentication));
