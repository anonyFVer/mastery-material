...
public class RecoveryDuringReplicationTests extends ESIndexLevelReplicationTestCase {
    ...
    public void testRecoveryToReplicaThatReceivedExtraDocument() throws Exception {
        try (ReplicationGroup shards = createGroup(2)) {
            ...
            remainingReplica.applyIndexOperationOnReplica(remainingReplica.getLocalCheckpoint() + 1, 1, VersionType.EXTERNAL, randomNonNegativeLong(), false, SourceToParse.source("index", "type", "replica", new BytesArray("{}"), XContentType.JSON), mapping -> {
            });
            ...
            if (extra) {
                promotedReplica.applyIndexOperationOnPrimary(Versions.MATCH_ANY, VersionType.INTERNAL, SourceToParse.source("index", "type", "primary", new BytesArray("{}"), XContentType.JSON), randomNonNegativeLong(), false, mapping -> {
                });
            }
    ...
    public void testRecoveryAfterPrimaryPromotion() throws Exception {
        try (ReplicationGroup shards = createGroup(2)) {
            ...
            assertThat(IndexShardTestCase.getGlobalCheckpointTracker(newPrimary).getTrackedLocalCheckpointForShard(newPrimary.routingEntry().allocationId().getId()).getLocalCheckpoint(), equalTo(totalDocs - 1L));
