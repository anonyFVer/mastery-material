...
public class PrimaryReplicaSyncer extends AbstractComponent {
    ...
    public void resync(final IndexShard indexShard, final ActionListener<ResyncTask> listener) {
        ...
        try {
            ...
            resync(shardId, indexShard.routingEntry().allocationId().getId(), indexShard.getPendingPrimaryTerm(), wrappedSnapshot, startingSeqNo, maxSeqNo, resyncListener);
        } catch (Exception e) {
            try {
    ...
    private void resync(final ShardId shardId, final String primaryAllocationId, final long primaryTerm, final Translog.Snapshot snapshot, long startingSeqNo, long maxSeqNo, ActionListener<ResyncTask> listener) {
        ...
        try {
            new SnapshotSender(logger, syncAction, resyncTask, shardId, primaryAllocationId, primaryTerm, snapshot, chunkSize.bytesAsInt(), startingSeqNo, maxSeqNo, wrappedListener).run();
        } catch (Exception e) {
    ...
    static class SnapshotSender extends AbstractRunnable implements ActionListener<ResyncReplicationResponse> {
        ...
        SnapshotSender(Logger logger, SyncAction syncAction, ResyncTask task, ShardId shardId, String primaryAllocationId, long primaryTerm, Translog.Snapshot snapshot, int chunkSizeInBytes, long startingSeqNo, long maxSeqNo, ActionListener<Void> listener) {
            this.logger = logger;
            this.syncAction = syncAction;
            this.task = task;
        ...
        protected void doRun() throws Exception {
            ...
            if (!operations.isEmpty() || trimmedAboveSeqNo != SequenceNumbers.UNASSIGNED_SEQ_NO) {
                ...
                ResyncReplicationRequest request = new ResyncReplicationRequest(shardId, trimmedAboveSeqNo, operations.toArray(EMPTY_ARRAY));
