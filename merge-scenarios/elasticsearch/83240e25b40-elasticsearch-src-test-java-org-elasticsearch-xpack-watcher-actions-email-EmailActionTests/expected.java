...
public class EmailActionTests extends ESTestCase {
    ...
    public void testExecute() throws Exception {
        ...
        ExecutableEmailAction executable = new ExecutableEmailAction(action, logger, service, engine, htmlSanitizer, emailAttachmentParser.getParsers());
        ...
        WatchExecutionContext ctx = mockExecutionContextBuilder("watch1").wid(wid).payload(payload).time("watch1", now).metadata(metadata).buildMock();
    ...
    public void testParser() throws Exception {
        ...
        DataAttachment dataAttachment = randomDataAttachment();
        XContentBuilder builder = jsonBuilder().startObject().field("account", "_account").field("profile", profile.name()).field("user", "_user").field("password", "_passwd").field("from", "from@domain").field("priority", priority.name());
        if (dataAttachment != null) {
        ...
        parser.nextToken();
        ExecutableEmailAction executable = new EmailActionFactory(Settings.EMPTY, emailService, engine, emailAttachmentParser).parseExecutable(randomAsciiOfLength(8), randomAsciiOfLength(3), parser);
    ...
    public void testParserSelfGenerated() throws Exception {
        ...
        XContentParser parser = createParser(JsonXContent.jsonXContent, bytes);
        parser.nextToken();
        ExecutableEmailAction parsed = new EmailActionFactory(Settings.EMPTY, service, engine, emailAttachmentParser).parseExecutable(randomAsciiOfLength(4), randomAsciiOfLength(10), parser);
        if (!hideSecrets) {
    ...
    public void testParserInvalid() throws Exception {
        ...
        parser.nextToken();
        try {
            new EmailActionFactory(Settings.EMPTY, emailService, engine, emailAttachmentsParser).parseExecutable(randomAsciiOfLength(3), randomAsciiOfLength(7), parser);
    ...
    public void testRequestAttachmentGetsAppendedToEmailAttachments() throws Exception {
        ...
        XContentBuilder builder = jsonBuilder().startObject().startObject("attachments").startObject(attachmentId).startObject("http").startObject("request").field("host", "localhost").field("port", 443).field("path", "/the/evil/test").endObject().endObject().endObject().endObject().endObject();
        XContentParser parser = createParser(builder);
        logger.info("JSON: {}", builder.string());
        parser.nextToken();
        EmailActionFactory emailActionFactory = createEmailActionFactory();
        ExecutableEmailAction executableEmailAction = emailActionFactory.parseExecutable(randomAsciiOfLength(3), randomAsciiOfLength(7), parser);
    ...
    public void testThatDataAttachmentGetsAttachedWithId() throws Exception {
        ...
        XContentBuilder builder = jsonBuilder().startObject().startObject("attachments").startObject(attachmentId).startObject("data").endObject().endObject().endObject().endObject();
        XContentParser parser = createParser(builder);
        logger.info("JSON: {}", builder.string());
        parser.nextToken();
        EmailActionFactory emailActionFactory = createEmailActionFactory();
    ...
    public void testThatOneFailedEmailAttachmentResultsInActionFailure() throws Exception {
        ...
        Map<String, String[]> headers = new HashMap<>(1);
        headers.put(HttpHeaders.Names.CONTENT_TYPE, new String[] { "plain/text" });
        when(httpClient.execute(any(HttpRequest.class))).thenReturn(new HttpResponse(200, "body", headers)).thenReturn(new HttpResponse(403));
        ...
        XContentBuilder builder = jsonBuilder().startObject().startObject("attachments").startObject("first").startObject("http").startObject("request").field("url", "http://localhost/first").endObject().endObject().endObject().startObject("second").startObject("http").startObject("request").field("url", "http://localhost/second").endObject().endObject().endObject().endObject().endObject();
        XContentParser parser = createParser(builder);
        parser.nextToken();
        ExecutableEmailAction executableEmailAction = new EmailActionFactory(Settings.EMPTY, emailService, engine, emailAttachmentsParser).parseExecutable(randomAsciiOfLength(3), randomAsciiOfLength(7), parser);
        DateTime now = DateTime.now(DateTimeZone.UTC);
        Wid wid = new Wid(randomAsciiOfLength(5), randomLong(), now);
        Map<String, Object> metadata = MapBuilder.<String, Object>newMapBuilder().put("_key", "_val").map();
        WatchExecutionContext ctx = mockExecutionContextBuilder("watch1").wid(wid).payload(new Payload.Simple()).time("watch1", now).metadata(metadata).buildMock();
        ...
        assertThat(failure.reason(), is("Watch[watch1] attachment[second] HTTP error status host[localhost], port[80], method[GET], path[/second], " + "status[403]"));
    }
    ...
    private EmailAttachments randomEmailAttachments() throws IOException {
        ...
        if ("http".equals(attachmentType)) {
            ...
            attachments.add(new HttpRequestAttachment(randomAsciiOfLength(10), template, randomBoolean(), randomFrom("my/custom-type", null)));
        } else if ("data".equals(attachmentType)) {
            attachments.add(new org.elasticsearch.xpack.notification.email.attachment.DataAttachment(randomAsciiOfLength(10), randomFrom(DataAttachment.JSON, DataAttachment.YAML)));
        }
