...
public class AuthorizationService extends AbstractComponent {
    ...
    public void authorize(Authentication authentication, String action, TransportRequest request, Role userRole, Role runAsRole) throws ElasticsearchSecurityException {
        ...
        if (isCompositeAction(action)) {
            ...
            if (permission.indices().check(action)) {
                ...
                setIndicesAccessControlResolver((indicesOptions, indices) -> {
                    ...
                    IndicesAccessControl indicesAccessControl = finalPermission.authorize(action, localIndices, metaData, fieldPermissionsCache);
                    if (!indicesAccessControl.isGranted()) {
                        throw denial(authentication, action, finalRequest, finalPermission.names(), specificIndices);
                    }
                    if (hasSecurityIndexAccess(indicesAccessControl) && MONITOR_INDEX_PREDICATE.test(action) == false && isSuperuser(authentication.getUser()) == false) {
        ...
        final MetaData metaData = clusterService.state().metaData();
        final AuthorizedIndices authorizedIndices = new AuthorizedIndices(authentication.getUser(), permission, action, metaData);
        final ResolvedIndices resolvedIndices = resolveIndexNames(authentication, action, request, request, metaData, authorizedIndices, permission);
    ...
    private ResolvedIndices resolveIndexNames(Authentication authentication, String action, Object indicesRequest, TransportRequest mainRequest, MetaData metaData, AuthorizedIndices authorizedIndices, Role permission) {
        try {
    ...
    ElasticsearchSecurityException denial(Authentication authentication, String action, TransportRequest request, String[] roleNames, @Nullable Set<String> specificIndices) {
    ...
    private void grant(Authentication authentication, String action, TransportRequest request, String[] roleNames, @Nullable Set<String> specificIndices) {
