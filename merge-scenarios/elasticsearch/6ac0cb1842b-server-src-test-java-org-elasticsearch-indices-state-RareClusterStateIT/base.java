...
public class RareClusterStateIT extends ESIntegTestCase {
    ...
    public void testDelayedMappingPropagationOnPrimary() throws Exception {
        Settings settings = Settings.builder().put(DiscoverySettings.COMMIT_TIMEOUT_SETTING.getKey(), "30s").put(DiscoverySettings.PUBLISH_TIMEOUT_SETTING.getKey(), "0s").build();
    ...
    public void testDelayedMappingPropagationOnReplica() throws Exception {
        final List<String> nodeNames = internalCluster().startNodes(2, Settings.builder().put(DiscoverySettings.COMMIT_TIMEOUT_SETTING.getKey(), "30s").put(DiscoverySettings.PUBLISH_TIMEOUT_SETTING.getKey(), "0s").build());
        assertFalse(client().admin().cluster().prepareHealth().setWaitForNodes("2").get().isTimedOut());
        final String master = internalCluster().getMasterName();
        assertThat(nodeNames, hasItem(master));
        String otherNode = null;
