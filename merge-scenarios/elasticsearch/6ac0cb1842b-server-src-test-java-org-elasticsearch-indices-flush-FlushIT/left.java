...
public class FlushIT extends ESIntegTestCase {
    ...
    private void indexDoc(Engine engine, String id) throws IOException {
        ...
        final Engine.IndexResult indexResult = engine.index(new Engine.Index(new Term("_id", Uid.encodeId(doc.id())), doc, engine.getLocalCheckpoint() + 1, 1L, 1L, null, Engine.Operation.Origin.REPLICA, randomLong(), -1L, false));
    ...
    public void testSyncedFlushSkipOutOfSyncReplicas() throws Exception {
        ...
        assertAcked(prepareCreate("test").setSettings(Settings.builder().put(IndexMetaData.SETTING_NUMBER_OF_SHARDS, 1).put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, numberOfReplicas)).get());
        ensureGreen();
        ...
        final List<IndexShard> indexShards = internalCluster().nodesInclude("test").stream().map(node -> internalCluster().getInstance(IndicesService.class, node).getShardOrNull(shardId)).collect(Collectors.toList());
        ...
        assertThat(partialResult.shardResponses().get(outOfSyncReplica.routingEntry()).failureReason, equalTo("out of sync replica; num docs on replica [" + (numDocs + extraDocs) + "]; num docs on primary [" + numDocs + "]"));
    ...
    public void testDoNotRenewSyncedFlushWhenAllSealed() throws Exception {
        ...
        IndexShard shard = internalCluster().getInstance(IndicesService.class, randomFrom(internalCluster().nodesInclude("test"))).getShardOrNull(shardId);
        if (randomBoolean()) {
