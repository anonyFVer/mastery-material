...
public class MasterService extends AbstractLifecycleComponent {
    ...
    protected PrioritizedEsThreadPoolExecutor createThreadPoolExecutor() {
        return EsExecutors.newSinglePrioritizing(nodeName + "/" + MASTER_UPDATE_THREAD_NAME, daemonThreadFactory(settings, MASTER_UPDATE_THREAD_NAME), threadPool.getThreadContext(), threadPool.scheduler());
    }
    ...
    public static boolean assertNotMasterUpdateThread(String reason) {
        assert isMasterUpdateThread() == false : "Expected current thread [" + Thread.currentThread() + "] to not be the master service thread. Reason: [" + reason + "]";
        return true;
    }
    ...
    protected void onPublicationSuccess(ClusterChangedEvent clusterChangedEvent, TaskOutputs taskOutputs, long startTimeNS) {
        ...
        TimeValue executionTime = TimeValue.timeValueMillis(Math.max(0, TimeValue.nsecToMSec(currentTimeInNanos() - startTimeNS)));
        logger.debug("processing [{}]: took [{}] done publishing updated cluster state (version: {}, uuid: {})", clusterChangedEvent.source(), executionTime, clusterChangedEvent.state().version(), clusterChangedEvent.state().stateUUID());
        warnAboutSlowTaskIfNeeded(executionTime, clusterChangedEvent.source());
    }

    protected void onPublicationFailed(ClusterChangedEvent clusterChangedEvent, TaskOutputs taskOutputs, long startTimeNS, Exception exception) {
