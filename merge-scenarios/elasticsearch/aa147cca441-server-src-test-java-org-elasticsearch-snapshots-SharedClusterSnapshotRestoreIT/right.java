...
public class SharedClusterSnapshotRestoreIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testRestoreSnapshotWithCorruptedIndexMetadata() throws Exception {
        ...
        RestoreSnapshotResponse restoreSnapshotResponse = client().admin().cluster().prepareRestoreSnapshot("test-repo", "test-snap").setIndices(nbDocsPerIndex.keySet().stream().filter(isRestorableIndex).toArray(String[]::new)).setRestoreGlobalState(randomBoolean()).setWaitForCompletion(true).get();
        ensureGreen();
