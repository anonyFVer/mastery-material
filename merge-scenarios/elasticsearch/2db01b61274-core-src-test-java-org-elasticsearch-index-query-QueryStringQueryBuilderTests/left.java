...
public class QueryStringQueryBuilderTests extends AbstractQueryTestCase<QueryStringQueryBuilder> {
    ...
    protected void doAssertLuceneQuery(QueryStringQueryBuilder queryBuilder, Query query, SearchContext context) throws IOException {
        if ("".equals(queryBuilder.queryString())) {
            ...
            assertThat(query, either(instanceOf(TermQuery.class)).or(instanceOf(AllTermQuery.class)).or(instanceOf(BooleanQuery.class)).or(instanceOf(DisjunctionMaxQuery.class)).or(instanceOf(PhraseQuery.class)));
        }
    }
    ...
    public void testFuzzyNumeric() throws Exception {
        ...
        assertEquals("Can only use fuzzy queries on keyword and text fields - not on [mapped_int] which is of type [integer]", e.getMessage());
        query.lenient(true);
    ...
    public void testPrefixNumeric() throws Exception {
        ...
        assertEquals("Can only use prefix queries on keyword and text fields - not on [mapped_int] which is of type [integer]", e.getMessage());
        query.lenient(true);
        query.toQuery(context);
    }
    ...
    public void testToQuerySplitOnWhitespace() throws IOException {
        assumeTrue("test runs only when at least a type is registered", getCurrentTypes().length > 0);
        {
            QueryStringQueryBuilder queryBuilder = new QueryStringQueryBuilder("foo bar").field(STRING_FIELD_NAME).field(STRING_FIELD_NAME_2).splitOnWhitespace(false);
            Query query = queryBuilder.toQuery(createShardContext());
            BooleanQuery bq1 = new BooleanQuery.Builder().add(new BooleanClause(new TermQuery(new Term(STRING_FIELD_NAME, "foo")), BooleanClause.Occur.SHOULD)).add(new BooleanClause(new TermQuery(new Term(STRING_FIELD_NAME, "bar")), BooleanClause.Occur.SHOULD)).build();
        ...
        {
            QueryStringQueryBuilder queryBuilder = new QueryStringQueryBuilder("mapped_string:other foo bar").field(STRING_FIELD_NAME).field(STRING_FIELD_NAME_2).splitOnWhitespace(false);
            Query query = queryBuilder.toQuery(createShardContext());
            BooleanQuery bq1 = new BooleanQuery.Builder().add(new BooleanClause(new TermQuery(new Term(STRING_FIELD_NAME, "foo")), BooleanClause.Occur.SHOULD)).add(new BooleanClause(new TermQuery(new Term(STRING_FIELD_NAME, "bar")), BooleanClause.Occur.SHOULD)).build();
            List<Query> disjuncts = new ArrayList<>();
            disjuncts.add(bq1);
            ...
            BooleanQuery expectedQuery = new BooleanQuery.Builder().add(disjunctionMaxQuery, BooleanClause.Occur.SHOULD).add(new TermQuery(new Term(STRING_FIELD_NAME, "other")), BooleanClause.Occur.SHOULD).build();
            assertThat(query, equalTo(expectedQuery));
        }
        {
            QueryStringQueryBuilder queryBuilder = new QueryStringQueryBuilder("foo OR bar").field(STRING_FIELD_NAME).field(STRING_FIELD_NAME_2).splitOnWhitespace(false);
            Query query = queryBuilder.toQuery(createShardContext());
            ...
            BooleanQuery expectedQuery = new BooleanQuery.Builder().add(new BooleanClause(maxQuery1, BooleanClause.Occur.SHOULD)).add(new BooleanClause(maxQuery2, BooleanClause.Occur.SHOULD)).build();
            assertThat(query, equalTo(expectedQuery));
        }
        {
            QueryStringQueryBuilder queryBuilder = new QueryStringQueryBuilder(">10 foo").field(INT_FIELD_NAME).splitOnWhitespace(false);
        ...
        {
            QueryStringQueryBuilder queryBuilder = new QueryStringQueryBuilder(">10 foo").field(INT_FIELD_NAME).splitOnWhitespace(false).lenient(true);
            Query query = queryBuilder.toQuery(createShardContext());
            BooleanQuery bq = new BooleanQuery.Builder().build();
            assertThat(bq, equalTo(query));
        }
        {
            QueryStringQueryBuilder queryBuilder = new QueryStringQueryBuilder("foo bar").field(STRING_FIELD_NAME).field(STRING_FIELD_NAME_2).splitOnWhitespace(true);
            Query query = queryBuilder.toQuery(createShardContext());
            List<Query> disjuncts1 = new ArrayList<>();
            disjuncts1.add(new TermQuery(new Term(STRING_FIELD_NAME, "foo")));
            disjuncts1.add(new TermQuery(new Term(STRING_FIELD_NAME_2, "foo")));
    ...
    public void testInvalidCombo() throws IOException {
        ...
        IllegalArgumentException exc = expectThrows(IllegalArgumentException.class, () -> builder.toQuery(createShardContext()));
        assertEquals(exc.getMessage(), "it is disallowed to disable [split_on_whitespace] if [auto_generate_phrase_queries] is activated");
    }
}