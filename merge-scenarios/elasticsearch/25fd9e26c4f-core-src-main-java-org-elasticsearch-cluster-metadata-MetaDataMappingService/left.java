...
public class MetaDataMappingService extends AbstractComponent {
    ...
    public void refreshMapping(final String index, final String indexUUID) {
        ...
        clusterService.submitStateUpdateTask("refresh-mapping", refreshTask, ClusterStateTaskConfig.build(Priority.HIGH), refreshExecutor, (source, e) -> logger.warn("failure during [{}]", e, source));
    }
                    ...
                    try {
                        for (Index index : request.indices()) {
                            ...
                            if (indicesService.hasIndex(indexMetaData.getIndex()) == false) {
                                ...
                                IndexService indexService = indicesService.createIndex(nodeServicesProvider, indexMetaData, Collections.emptyList(), shardId -> {
                                });
