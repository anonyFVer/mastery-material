...
public abstract class ESIndexLevelReplicationTestCase extends IndexShardTestCase {
    ...
    protected class ReplicationGroup implements AutoCloseable, Iterable<IndexShard> {
        ...
        public synchronized IndexShard addReplica() throws IOException {
            final IndexShard replica = newShard(shardId, false, "s" + replicaId.incrementAndGet(), indexMetaData, () -> {
                throw new AssertionError("replicas can't sync global checkpoint");
            }, null);
    ...
    abstract class ReplicationAction<Request extends ReplicationRequest<Request>, ReplicaRequest extends ReplicationRequest<ReplicaRequest>, Response extends ReplicationResponse> {
        ...
        public ReplicationAction(Request request, ActionListener<Response> listener, ReplicationGroup group, String opType) {
        ...
        public void execute() throws Exception {
            new ReplicationOperation<Request, ReplicaRequest, PrimaryResult>(request, new PrimaryRef(), new ActionListener<PrimaryResult>() {
                ...
                protected Set<String> getInSyncAllocationIds(ShardId shardId, ClusterState clusterState) {
                    return replicationGroup.shardRoutings().stream().filter(ShardRouting::active).map(r -> r.allocationId().getId()).collect(Collectors.toSet());
                }
            }.execute();
        }
        ...
        class ReplicasRef implements ReplicationOperation.Replicas<ReplicaRequest> {
            ...
            @Override
            public void performOn(ShardRouting replicaRouting, ReplicaRequest request, ActionListener<ReplicationOperation.ReplicaResponse> listener) {
                try {
    ...
    class IndexingAction extends ReplicationAction<IndexRequest, IndexRequest, IndexResponse> {
        ...
        protected PrimaryResult performOnPrimary(IndexShard primary, IndexRequest request) throws Exception {
            ...
            IndexResponse response = new IndexResponse(primary.shardId(), request.type(), request.id(), indexResult.getSeqNo(), indexResult.getVersion(), indexResult.isCreated());
    ...
    class GlobalCheckpointSync extends ReplicationAction<GlobalCheckpointSyncAction.PrimaryRequest, GlobalCheckpointSyncAction.ReplicaRequest, ReplicationResponse> {

        public GlobalCheckpointSync(ActionListener<ReplicationResponse> listener, ReplicationGroup replicationGroup) {
            super(new GlobalCheckpointSyncAction.PrimaryRequest(replicationGroup.getPrimary().shardId()), listener, replicationGroup, "global_ckp");
        }
        ...
        protected PrimaryResult performOnPrimary(IndexShard primary, GlobalCheckpointSyncAction.PrimaryRequest request) throws Exception {
            return new PrimaryResult(new GlobalCheckpointSyncAction.ReplicaRequest(request, primary.getGlobalCheckpoint()), new ReplicationResponse());
        }
