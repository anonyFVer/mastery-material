...
public abstract class TransportReplicationAction<Request extends ReplicationRequest<Request>, ReplicaRequest extends ReplicationRequest<ReplicaRequest>, Response extends ReplicationResponse> extends TransportAction<Request, Response> {
    ...
    private final class AsyncReplicaAction extends AbstractRunnable implements ActionListener<ShardReference> {
        ...
        public void onResponse(ShardReference replica) {
            try {
                ...
                Releasables.closeWhileHandlingException(replica);
    ...
    protected void acquireReplicaOperationLock(ShardId shardId, long primaryTerm, final String allocationId, ActionListener<ShardReference> onLockAcquired) {
        IndexService indexService = indicesService.indexServiceSafe(shardId.getIndex());
        IndexShard indexShard = indexService.getShard(shardId.id());
        final String actualAllocationId = indexShard.routingEntry().allocationId().getId();
        if (actualAllocationId.equals(allocationId) == false) {
    ...
    final class ReplicasProxy implements ReplicationOperation.Replicas<ReplicaRequest> {
        ...
        public void performOn(ShardRouting replica, ReplicaRequest request, ActionListener<ReplicationOperation.ReplicaResponse> listener) {
            ...
            transportService.sendRequest(node, transportReplicaAction, new ConcreteShardRequest<>(request, replica.allocationId().getId()), transportOptions, new ActionListenerResponseHandler<>(listener, ReplicaResponse::new));
        }
