...
public class SecuritySearchOperationListenerTests extends ESTestCase {
    ...
    public void testValidateSearchContext() throws Exception {
        ...
        try (StoredContext ignore = threadContext.newStoredContext(false)) {
            ...
            verify(auditTrailService).accessDenied(authentication.getUser(), "action", request, null);
        }
        try (StoredContext ignore = threadContext.newStoredContext(false)) {
            final String nodeName = randomBoolean() ? "node" : randomAlphaOfLengthBetween(1, 8);
            final String realmName = randomBoolean() ? "realm" : randomAlphaOfLengthBetween(1, 16);
    ...
    public void testEnsuredAuthenticatedUserIsSame() {
        ...
        SearchContextMissingException e = expectThrows(SearchContextMissingException.class, () -> ensureAuthenticatedUserIsSame(original, differentRealmType, auditTrail, id, action, request));
        ...
        e = expectThrows(SearchContextMissingException.class, () -> ensureAuthenticatedUserIsSame(original, differentUser, auditTrail, id, action, request));
        assertEquals(id, e.id());
        ...
        e = expectThrows(SearchContextMissingException.class, () -> ensureAuthenticatedUserIsSame(original, diffRunAs, auditTrail, id, action, request));
        assertEquals(id, e.id());
        ...
        e = expectThrows(SearchContextMissingException.class, () -> ensureAuthenticatedUserIsSame(runAs, runAsDiffType, auditTrail, id, action, request));
        assertEquals(id, e.id());
