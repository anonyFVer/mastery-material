...
public class MlMetadata implements MetaData.Custom {
    ...
    private static <T extends ToXContent> void mapValuesToXContent(ParseField field, Map<String, T> map, XContentBuilder builder, Params params) throws IOException {
    ...
    public static class MlMetadataDiff implements NamedDiff<MetaData.Custom> {
        ...
        public MlMetadataDiff(StreamInput in) throws IOException {
            ...
            this.datafeeds = DiffableUtils.readJdkMapDiff(in, DiffableUtils.getStringKeySerializer(), DatafeedConfig::new, MlMetadataDiff::readSchedulerDiffFrom);
        }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(jobs, that.jobs) && Objects.equals(datafeeds, that.datafeeds);
    }
    ...
    public static class Builder {
        ...
        public Builder deleteJob(String jobId, PersistentTasksInProgress tasks) {
            ...
            if (datafeed.isPresent()) {
                throw ExceptionsHelper.conflictStatusException("Cannot delete job [" + jobId + "] while datafeed [" + datafeed.get().getId() + "] refers to it");
            }
            JobState jobState = MlMetadata.getJobState(jobId, tasks);
            if (jobState.isAnyOf(JobState.CLOSED, JobState.FAILED) == false) {
                throw ExceptionsHelper.conflictStatusException("Unexpected job state [" + jobState + "], expected [" + JobState.CLOSED + "]");
            }
        ...
        private void checkJobIsAvailableForDatafeed(String jobId) {
            ...
            if (existingDatafeed.isPresent()) {
                throw ExceptionsHelper.conflictStatusException("A datafeed [" + existingDatafeed.get().getId() + "] already exists for job [" + jobId + "]");
            }
        }
        ...
        public Builder updateDatafeed(DatafeedUpdate update, PersistentTasksInProgress persistentTasksInProgress) {
            ...
            checkDatafeedIsStopped(() -> Messages.getMessage(Messages.DATAFEED_CANNOT_UPDATE_IN_CURRENT_STATE, datafeedId, DatafeedState.STARTED), datafeedId, persistentTasksInProgress);
        ...
        public Builder removeDatafeed(String datafeedId, PersistentTasksInProgress persistentTasksInProgress) {
            ...
            checkDatafeedIsStopped(() -> Messages.getMessage(Messages.DATAFEED_CANNOT_DELETE_IN_CURRENT_STATE, datafeedId, DatafeedState.STARTED), datafeedId, persistentTasksInProgress);
    ...
    @Nullable
    public static PersistentTasksInProgress.PersistentTaskInProgress<?> getJobTask(String jobId, @Nullable PersistentTasksInProgress tasks) {
        if (tasks != null) {
    ...
    @Nullable
    public static PersistentTasksInProgress.PersistentTaskInProgress<?> getDatafeedTask(String datafeedId, @Nullable PersistentTasksInProgress tasks) {
        if (tasks != null) {
            Predicate<PersistentTasksInProgress.PersistentTaskInProgress<?>> p = t -> {
                StartDatafeedAction.Request storedRequest = (StartDatafeedAction.Request) t.getRequest();
                return storedRequest.getDatafeedId().equals(datafeedId);
