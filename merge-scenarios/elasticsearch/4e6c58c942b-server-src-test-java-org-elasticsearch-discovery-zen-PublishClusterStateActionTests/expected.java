...
public class PublishClusterStateActionTests extends ESTestCase {
    ...
    public static MockNode createMockNode(String name, final Settings basSettings, @Nullable ClusterStateListener listener, ThreadPool threadPool, Logger logger, Map<String, MockNode> nodes) throws Exception {
        final Settings settings = Settings.builder().put("name", name).put(TransportService.TRACE_LOG_INCLUDE_SETTING.getKey(), "").put(TransportService.TRACE_LOG_EXCLUDE_SETTING.getKey(), "NOTHING").put(basSettings).build();
    ...
    private static MockPublishAction buildPublishClusterStateAction(Settings settings, MockTransportService transportService, PublishClusterStateAction.IncomingClusterStateListener listener) {
    ...
    public void testSerializationFailureDuringDiffPublishing() throws Exception {
        ...
        previousClusterState = clusterState;
        clusterState = ClusterState.builder(clusterState).blocks(ClusterBlocks.builder().addGlobalBlock(MetaData.CLUSTER_READ_ONLY_BLOCK)).incrementVersion().build();
        ClusterState unserializableClusterState = new ClusterState(clusterState.term(), clusterState.version(), clusterState.stateUUID(), clusterState) {
    ...
    private void assertPublishClusterStateStats(String description, MockNode node, long expectedFull, long expectedIncompatibleDiffs, long expectedCompatibleDiffs) {
        ...
        assertThat(description + ": compatible cluster state diffs", stats.getCompatibleClusterStateDiffReceivedCount(), equalTo(expectedCompatibleDiffs));
    }
    ...
    public void testPublishClusterStateStats() throws Exception {
        ...
        nodeA.resetMasterId();
        nodeB.resetMasterId();
        discoveryNodes = DiscoveryNodes.builder(discoveryNodes).add(nodeA.discoveryNode).add(nodeB.discoveryNode).masterNodeId(nodeB.discoveryNode.getId()).localNodeId(nodeB.discoveryNode.getId()).build();
        previousClusterState = ClusterState.builder(new ClusterName("test")).nodes(discoveryNodes).build();
        clusterState = ClusterState.builder(clusterState).nodes(discoveryNodes).incrementVersion().build();
        publishStateAndWait(nodeB.action, clusterState, previousClusterState);
    ...
    public static class MockPublishAction extends PublishClusterStateAction {
        ...
        public MockPublishAction(TransportService transportService, NamedWriteableRegistry namedWriteableRegistry, IncomingClusterStateListener listener, DiscoverySettings discoverySettings) {
