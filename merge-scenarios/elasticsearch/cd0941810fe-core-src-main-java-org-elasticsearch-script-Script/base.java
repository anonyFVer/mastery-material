...
public final class Script implements ToXContentObject, Writeable {
    ...
    private static final class Builder {
        ...
        private Script build(String defaultLang) {
            ...
            if (type == ScriptType.INLINE) {
                ...
                if (idOrCode == null) {
                    throw new IllegalArgumentException("must specify <code> for an [" + ScriptType.FILE.getParseField().getPreferredName() + "] script");
                }
                if (options.isEmpty()) {
                    options = null;
                } else {
                    throw new IllegalArgumentException("field [" + OPTIONS_PARSE_FIELD.getPreferredName() + "] " + "cannot be specified using a [" + ScriptType.FILE.getParseField().getPreferredName() + "] script");
                }
            }
    ...
    public Script(ScriptType type, String lang, String idOrCode, Map<String, String> options, Map<String, Object> params) {
        ...
        if (type == ScriptType.INLINE) {
            ...
            this.lang = Objects.requireNonNull(lang);
            if (options != null) {
                throw new IllegalStateException("options must be null for [" + ScriptType.FILE.getParseField().getPreferredName() + "] scripts");
            }
            this.options = null;
        } else {
