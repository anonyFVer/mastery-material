...
public class ClusterStatsMonitoringDocTests extends BaseMonitoringDocTestCase<ClusterStatsMonitoringDoc> {
    ...
    public void setUp() throws Exception {
        ...
        license = License.builder().uid(randomAlphaOfLength(5)).type(randomFrom(License.OperationMode.values()).name()).issuer(randomAlphaOfLength(5)).issuedTo(randomAlphaOfLength(5)).issueDate(timestamp).expiryDate(timestamp + randomIntBetween(1, 10) * 1_000L).maxNodes(randomIntBetween(1, 5)).build();
        final DiscoveryNode masterNode = masterNode();
        final DiscoveryNodes.Builder builder = DiscoveryNodes.builder().masterNodeId(masterNode.getId()).localNodeId(masterNode.getId()).add(masterNode);
        when(clusterState.nodes()).thenReturn(builder.build());
    }

    @Override
    protected ClusterStatsMonitoringDoc createMonitoringDoc(String cluster, long timestamp, long interval, MonitoringDoc.Node node, MonitoredSystem system, String type, String id) {
        return new ClusterStatsMonitoringDoc(cluster, timestamp, interval, node, clusterName, version, clusterStatus, license, apmIndicesExist, usages, clusterStats, clusterState, needToEnableTLS);
    }
    ...
    public void testConstructorClusterNameMustNotBeNull() {
        expectThrows(NullPointerException.class, () -> new ClusterStatsMonitoringDoc(cluster, timestamp, interval, node, null, version, clusterStatus, license, apmIndicesExist, usages, clusterStats, clusterState, needToEnableTLS));
    }

    public void testConstructorVersionMustNotBeNull() {
        expectThrows(NullPointerException.class, () -> new ClusterStatsMonitoringDoc(cluster, timestamp, interval, node, clusterName, null, clusterStatus, license, apmIndicesExist, usages, clusterStats, clusterState, needToEnableTLS));
    }

    public void testConstructorClusterHealthStatusMustNotBeNull() {
        expectThrows(NullPointerException.class, () -> new ClusterStatsMonitoringDoc(cluster, timestamp, interval, node, clusterName, version, null, license, apmIndicesExist, usages, clusterStats, clusterState, needToEnableTLS));
    }
    ...
    public void testToXContent() throws IOException {
        ...
        final DiscoveryNode discoveryNode = new DiscoveryNode("_node_name", "_node_id", "_ephemeral_id", "_host_name", "_host_address", transportAddress, singletonMap("attr", "value"), singleton(DiscoveryNode.Role.MASTER), Version.V_6_0_0_beta1);
        final ClusterState clusterState = ClusterState.builder(clusterName).metaData(MetaData.builder().clusterUUID(clusterUuid).build()).stateUUID("_state_uuid").version(12L).nodes(DiscoveryNodes.builder().masterNodeId("_node").localNodeId("_node").add(discoveryNode).build()).build();
        final License license = License.builder().uid("442ca961-9c00-4bb2-b5c9-dfaacd547403").type("trial").issuer("elasticsearch").issuedTo("customer").issueDate(1451606400000L).expiryDate(1502107402133L).maxNodes(2).build();
        ...
        when(mockNodeInfo.getSettings()).thenReturn(Settings.builder().put(NetworkModule.TRANSPORT_TYPE_KEY, "_transport").put(NetworkModule.HTTP_TYPE_KEY, "_http").build());
        ...
        final PluginInfo pluginInfo = new PluginInfo("_plugin", "_plugin_desc", "_plugin_version", Version.CURRENT, "1.8", "_plugin_class", Collections.emptyList(), false);
        ...
        final ShardRouting shardRouting = ShardRouting.newUnassigned(shardId, true, RecoverySource.ExistingStoreRecoverySource.INSTANCE, unassignedInfo);
