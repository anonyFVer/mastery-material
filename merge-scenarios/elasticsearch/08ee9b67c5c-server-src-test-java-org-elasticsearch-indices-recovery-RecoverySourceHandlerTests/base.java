...
public class RecoverySourceHandlerTests extends ESTestCase {
    ...
    public void testSendFiles() throws Throwable {
        ...
        RecoverySourceHandler handler = new RecoverySourceHandler(null, null, request, recoverySettings.getChunkSize().bytesAsInt(), Settings.EMPTY);
    ...
    public void testSendSnapshotSendsOps() throws IOException {
        ...
        final RecoverySourceHandler handler = new RecoverySourceHandler(shard, recoveryTarget, request, fileChunkSizeInBytes, Settings.EMPTY);
        ...
        RecoverySourceHandler.SendSnapshotResult result = handler.sendSnapshot(startingSeqNo, requiredStartingSeqNo, endingSeqNo, new Translog.Snapshot() {
        ...
        if (endingSeqNo >= requiredStartingSeqNo + 1) {
            List<Translog.Operation> requiredOps = operations.subList(0, operations.size() - 1).stream().filter(o -> o.seqNo() >= requiredStartingSeqNo && o.seqNo() <= endingSeqNo).collect(Collectors.toList());
            List<Translog.Operation> opsToSkip = randomSubsetOf(randomIntBetween(1, requiredOps.size()), requiredOps);
            expectThrows(IllegalStateException.class, () -> handler.sendSnapshot(startingSeqNo, requiredStartingSeqNo, endingSeqNo, new Translog.Snapshot() {
    ...
    private Engine.Index getIndex(final String id) {
        ...
        final ParsedDocument doc = new ParsedDocument(versionField, seqID, id, type, null, Arrays.asList(document), source, XContentType.JSON, null);
    ...
    public void testThrowExceptionOnPrimaryRelocatedBeforePhase1Started() throws IOException {
        ...
        final RecoverySourceHandler handler = new RecoverySourceHandler(shard, mock(RecoveryTargetHandler.class), request, recoverySettings.getChunkSize().bytesAsInt(), Settings.EMPTY) {
