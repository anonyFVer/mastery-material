    ...
    <T> void runTasksForExecutor(ClusterStateTaskExecutor<T> executor) {
        ...
        try {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("failed to execute cluster state update in [{}], state:\nversion [{}], source [{}]\n{}{}{}", e, executionTime, previousClusterState.version(), tasksSummary, previousClusterState.nodes().prettyPrint(), previousClusterState.routingTable().prettyPrint(), previousClusterState.getRoutingNodes().prettyPrint());
            }
        ...
        try {
            ...
            TimeValue executionTime = TimeValue.timeValueMillis(Math.max(0, TimeValue.nsecToMSec(currentTimeInNanos() - startTimeNS)));
            logger.warn("failed to apply updated cluster state in [{}]:\nversion [{}], uuid [{}], source [{}]\n{}", e, executionTime, newClusterState.version(), newClusterState.stateUUID(), tasksSummary, newClusterState.prettyPrint());
        }
    }
    ...
    private static class SafeClusterStateTaskListener implements ClusterStateTaskListener {
        ...
        public void clusterStateProcessed(String source, ClusterState oldState, ClusterState newState) {
            try {
                ...
                logger.error("exception thrown by listener while notifying of cluster state processed from [{}], old cluster state:\n" + "{}\nnew cluster state:\n{}", e, source, oldState.prettyPrint(), newState.prettyPrint());
            }
        }
    }
