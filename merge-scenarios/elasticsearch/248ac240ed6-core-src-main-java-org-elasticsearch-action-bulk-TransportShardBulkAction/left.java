    ...
    private Translog.Location executeBulkItemRequest(IndexMetaData metaData, IndexShard indexShard, BulkShardRequest request, long[] preVersions, VersionType[] preVersionTypes, Translog.Location location, int requestIndex) {
        ...
        try {
            ...
            if (ExceptionsHelper.status(e) == RestStatus.CONFLICT) {
                logger.trace("{} failed to execute bulk item ({}) {}", e, request.shardId(), documentRequest.opType().getLowercase(), request);
            } else {
                logger.debug("{} failed to execute bulk item ({}) {}", e, request.shardId(), documentRequest.opType().getLowercase(), request);
            }
