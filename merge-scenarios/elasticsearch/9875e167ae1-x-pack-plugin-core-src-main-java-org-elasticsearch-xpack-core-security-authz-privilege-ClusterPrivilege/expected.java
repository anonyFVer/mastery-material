...
public final class ClusterPrivilege extends Privilege {
    ...
    private static final Automaton MANAGE_SAML_AUTOMATON = patterns("cluster:admin/xpack/security/saml/*", InvalidateTokenAction.NAME, RefreshTokenAction.NAME);
    ...
    private static final Automaton MANAGE_CCR_AUTOMATON = patterns("cluster:admin/xpack/ccr/*", ClusterStateAction.NAME, HasPrivilegesAction.NAME);

    private static final Automaton CREATE_SNAPSHOT_AUTOMATON = patterns(CreateSnapshotAction.NAME, SnapshotsStatusAction.NAME + "*", GetSnapshotsAction.NAME, SnapshotsStatusAction.NAME, GetRepositoriesAction.NAME);
    ...
    private static final Automaton MANAGE_SLM_AUTOMATON = patterns("cluster:admin/slm/*", StartILMAction.NAME, StopILMAction.NAME, GetStatusAction.NAME);
    ...
    public static final ClusterPrivilege MONITOR_DATA_FRAME = new ClusterPrivilege("monitor_data_frame_transforms", MONITOR_DATA_FRAME_AUTOMATON);
    ...
    public static final ClusterPrivilege MANAGE_DATA_FRAME = new ClusterPrivilege("manage_data_frame_transforms", MANAGE_DATA_FRAME_AUTOMATON);
    ...
    private static final Map<String, ClusterPrivilege> VALUES = Map.ofEntries(entry("none", NONE), entry("all", ALL), entry("monitor", MONITOR), entry("monitor_ml", MONITOR_ML), entry("monitor_data_frame_transforms", MONITOR_DATA_FRAME), entry("monitor_watcher", MONITOR_WATCHER), entry("monitor_rollup", MONITOR_ROLLUP), entry("manage", MANAGE), entry("manage_ml", MANAGE_ML), entry("manage_data_frame_transforms", MANAGE_DATA_FRAME), entry("manage_token", MANAGE_TOKEN), entry("manage_watcher", MANAGE_WATCHER), entry("manage_index_templates", MANAGE_IDX_TEMPLATES), entry("manage_ingest_pipelines", MANAGE_INGEST_PIPELINES), entry("transport_client", TRANSPORT_CLIENT), entry("manage_security", MANAGE_SECURITY), entry("manage_saml", MANAGE_SAML), entry("manage_oidc", MANAGE_OIDC), entry("manage_api_key", MANAGE_API_KEY), entry("manage_pipeline", MANAGE_PIPELINE), entry("manage_rollup", MANAGE_ROLLUP), entry("manage_ccr", MANAGE_CCR), entry("read_ccr", READ_CCR), entry("create_snapshot", CREATE_SNAPSHOT), entry("manage_ilm", MANAGE_ILM), entry("read_ilm", READ_ILM), entry("manage_slm", MANAGE_SLM), entry("read_slm", READ_SLM), entry("manage_enrich", MANAGE_ENRICH));
