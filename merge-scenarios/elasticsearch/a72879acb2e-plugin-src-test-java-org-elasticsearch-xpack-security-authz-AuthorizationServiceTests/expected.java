...
public class AuthorizationServiceTests extends ESTestCase {
    ...
    public void testAuthorizationOfIndividualBulkItems() {
        ...
        roleMap.put("my-role", new RoleDescriptor("my-role", null, new IndicesPrivileges[] { IndicesPrivileges.builder().indices("concrete-index").privileges("all").build(), IndicesPrivileges.builder().indices("alias-1").privileges("index").build(), IndicesPrivileges.builder().indices("alias-2").privileges("delete").build() }, null));
        mockEmptyMetaData();
        authorize(createAuthentication(user), action, request);
        verify(auditTrail).accessDenied(user, DeleteAction.NAME, request, null);
        verify(auditTrail).accessDenied(user, IndexAction.NAME, request, null);
        verify(auditTrail).accessGranted(user, action, request, null);
        verifyNoMoreInteractions(auditTrail);
    }
    ...
    public void testAuthorizationOfIndividualBulkItemsWithDateMath() {
        ...
        final TransportRequest request = new BulkShardRequest(shardId, WriteRequest.RefreshPolicy.IMMEDIATE, items);
        User user = new User("user", "my-role");
        roleMap.put("my-role", new RoleDescriptor("my-role", null, new IndicesPrivileges[] { IndicesPrivileges.builder().indices("datemath-*").privileges("index").build() }, null));
        mockEmptyMetaData();
        authorize(createAuthentication(user), action, request);
        verify(auditTrail, Mockito.times(2)).accessDenied(user, DeleteAction.NAME, request, null);
        verify(auditTrail).accessGranted(user, action, request, null);
        verifyNoMoreInteractions(auditTrail);
    }
