...
public class IndexAuditTrail extends AbstractComponent implements AuditTrail, ClusterStateListener {
    ...
    public void start(boolean master) {
        if (state.compareAndSet(State.INITIALIZED, State.STARTING)) {
            ...
            if (indexToRemoteCluster) {
                client.admin().cluster().prepareState().execute(new ActionListener<ClusterStateResponse>() {
                    ...
                    public void onResponse(ClusterStateResponse clusterStateResponse) {
                        ...
                        if (canStart(clusterStateResponse.getState(), currentMaster)) {
                            if (currentMaster) {
                                putTemplate(customAuditIndexSettings(settings), ActionListener.wrap((v) -> innerStart(), (e) -> state.set(State.FAILED)));
                            } else {
                                innerStart();
                            }
                        } else {
                            if (state.compareAndSet(State.STARTING, State.INITIALIZED) == false) {
                                throw new IllegalStateException("state transition from starting to initialized failed, current value: " + state.get());
                            }
                            String indexName = getIndexName();
                            client.admin().cluster().prepareHealth().setIndices().setWaitForYellowStatus().execute(ActionListener.wrap((x) -> start(master), (e) -> logger.error("failed to get wait for yellow status on index [" + indexName + "]", e)));
                        }
                    }
                ...
                putTemplate(customAuditIndexSettings(settings), ActionListener.wrap((v) -> innerStart(), (e) -> {
    ...
    public void anonymousAccessDenied(String action, TransportMessage message) {
        if (events.contains(ANONYMOUS_ACCESS_DENIED)) {
            try {
                enqueue(message("anonymous_access_denied", action, (User) null, null, indices(message), message), "anonymous_access_denied");
            } catch (Exception e) {
    ...
    private void initializeBulkProcessor() {
        ...
        bulkProcessor = BulkProcessor.builder(client, new BulkProcessor.Listener() {
            ...
            public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
                logger.error((Supplier<?>) () -> new ParameterizedMessage("failed to bulk index audit events: [{}]", failure.getMessage()), failure);
            }
    ...
    public void clusterChanged(ClusterChangedEvent clusterChangedEvent) {
        ...
        if (state() == State.STARTED && clusterChangedEvent.localNodeMaster() && clusterChangedEvent.state().metaData().templates().get(INDEX_TEMPLATE_NAME) == null && putTemplatePending.compareAndSet(false, true)) {
            logger.debug("security audit index template [{}] does not exist. it may have been deleted - putting the template", INDEX_TEMPLATE_NAME);
            putTemplate(customAuditIndexSettings(settings), new ActionListener<Void>() {
                ...
                public void onFailure(Exception e) {
                    putTemplatePending.set(false);
                    logger.error((Supplier<?>) () -> new ParameterizedMessage("failed to update security audit index template [{}]", INDEX_TEMPLATE_NAME), e);
                }
            });
        }
    }
