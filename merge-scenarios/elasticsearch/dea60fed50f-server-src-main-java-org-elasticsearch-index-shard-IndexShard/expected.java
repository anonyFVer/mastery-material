...
public class IndexShard extends AbstractIndexShardComponent implements IndicesClusterStateService.Shard {
    ...
    public IndexShard(ShardRouting shardRouting, IndexSettings indexSettings, ShardPath path, Store store, Supplier<Sort> indexSortSupplier, IndexCache indexCache, MapperService mapperService, SimilarityService similarityService, @Nullable EngineFactory engineFactory, IndexEventListener indexEventListener, IndexSearcherWrapper indexSearcherWrapper, ThreadPool threadPool, BigArrays bigArrays, Engine.Warmer warmer, List<SearchOperationListener> searchOperationListener, List<IndexingOperationListener> listeners, Runnable globalCheckpointSyncer, CircuitBreakerService circuitBreakerService) throws IOException {
        ...
        this.globalCheckpointTracker = new GlobalCheckpointTracker(shardId, shardRouting.allocationId().getId(), indexSettings, SequenceNumbers.UNASSIGNED_SEQ_NO);
                ...
                if (newPrimaryTerm != primaryTerm) {
                    ...
                    indexShardOperationPermits.asyncBlockOperations(30, TimeUnit.MINUTES, () -> {
                        ...
                        try {
                            ...
                            globalCheckpointTracker.updateLocalCheckpoint(currentRouting.allocationId().getId(), getEngine().getLocalCheckpointTracker().getCheckpoint());
    ...
    private void innerOpenEngineAndTranslog(final EngineConfig.OpenMode openMode, final boolean forceNewHistoryUUID) throws IOException {
        ...
        if (openMode == EngineConfig.OpenMode.OPEN_INDEX_AND_TRANSLOG) {
            globalCheckpointTracker.updateGlobalCheckpointOnReplica(Translog.readGlobalCheckpoint(translogConfig.getTranslogPath()), "read from translog checkpoint");
        }
