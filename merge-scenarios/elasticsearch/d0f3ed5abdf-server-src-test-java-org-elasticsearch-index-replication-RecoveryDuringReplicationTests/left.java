...
public class RecoveryDuringReplicationTests extends ESIndexLevelReplicationTestCase {
    ...
    public void testRecoveryToReplicaThatReceivedExtraDocument() throws Exception {
        try (ReplicationGroup shards = createGroup(2)) {
            ...
            remainingReplica.applyIndexOperationOnReplica(remainingReplica.getLocalCheckpoint() + 1, 1, VersionType.EXTERNAL, randomNonNegativeLong(), false, SourceToParse.source("index", "type", "replica", new BytesArray("{}"), XContentType.JSON));
            ...
            if (extra) {
                promotedReplica.applyIndexOperationOnPrimary(Versions.MATCH_ANY, VersionType.INTERNAL, SourceToParse.source("index", "type", "primary", new BytesArray("{}"), XContentType.JSON), randomNonNegativeLong(), false);
            }
            final IndexShard recoveredReplica = shards.addReplicaWithExistingPath(remainingReplica.shardPath(), remainingReplica.routingEntry().currentNodeId());
            shards.recoverReplica(recoveredReplica);
            assertThat(recoveredReplica.recoveryState().getIndex().fileDetails(), empty());
    ...
    public void testRecoveryAfterPrimaryPromotion() throws Exception {
        ...
        try (ReplicationGroup shards = createGroup(2, settings)) {
            ...
            assertThat(IndexShardTestCase.getReplicationTracker(newPrimary).getTrackedLocalCheckpointForShard(newPrimary.routingEntry().allocationId().getId()).getLocalCheckpoint(), equalTo(totalDocs - 1L));
            ...
            if (expectSeqNoRecovery == false) {
                ...
                builder.settings(Settings.builder().put(newPrimary.indexSettings().getSettings()).put(IndexSettings.INDEX_TRANSLOG_RETENTION_AGE_SETTING.getKey(), "-1").put(IndexSettings.INDEX_TRANSLOG_RETENTION_SIZE_SETTING.getKey(), "-1").put(IndexSettings.INDEX_SOFT_DELETES_RETENTION_OPERATIONS_SETTING.getKey(), 0));
    ...
    public void testResyncAfterPrimaryPromotion() throws Exception {
        ...
        try (ReplicationGroup shards = new ReplicationGroup(buildIndexMetaData(2, mappings))) {
            ...
            for (int i = 0; i < initialDocs; i++) {
                final IndexRequest indexRequest = new IndexRequest(index.getName(), "type", "initial_doc_" + i).source("{ \"f\": \"normal\"}", XContentType.JSON);
            ...
            for (int i = 0; i < extraDocs; i++) {
                final IndexRequest indexRequest = new IndexRequest(index.getName(), "type", "extra_doc_" + i).source("{ \"f\": \"normal\"}", XContentType.JSON);
                final BulkShardRequest bulkShardRequest = indexOnPrimary(indexRequest, oldPrimary);
            ...
            for (int i = 0; i < extraDocsToBeTrimmed; i++) {
                final IndexRequest indexRequest = new IndexRequest(index.getName(), "type", "extra_trimmed_" + i).source("{ \"f\": \"trimmed\"}", XContentType.JSON);
                final BulkShardRequest bulkShardRequest = indexOnPrimary(indexRequest, oldPrimary);
            ...
            for (IndexShard replica : replicas) {
                ...
                assertThat(replica.routingEntry() + " has to have extra docs", temp, extraDocsToBeTrimmed > 0 ? not(empty()) : empty());
            }
    ...
    public void testWaitForPendingSeqNo() throws Exception {
        ...
        try (ReplicationGroup shards = new ReplicationGroup(metaData) {
            ...
            recoveryFuture.get();
            assertThat(newReplica.recoveryState().getIndex().fileDetails(), empty());
            assertThat(newReplica.recoveryState().getTranslog().recoveredOperations(), both(greaterThanOrEqualTo(docs - indexedDuringRecovery)).and(lessThanOrEqualTo(docs)));
            shards.assertAllEqual(docs);
        } finally {
    ...
    public void testCheckpointsAndMarkingInSync() throws Exception {
        ...
        try (ReplicationGroup shards = new ReplicationGroup(metaData) {
            ...
            final Future<Void> recoveryFuture = shards.asyncRecoverReplica(replica, (indexShard, node) -> new RecoveryTarget(indexShard, node, recoveryListener, l -> {
            }) {

                @Override
                public long indexTranslogOperations(final List<Translog.Operation> operations, final int totalTranslogOps) throws IOException {
    ...
    public static class BlockingTarget extends RecoveryTarget {
        ...
        static final EnumSet<RecoveryState.Stage> SUPPORTED_STAGES = EnumSet.of(RecoveryState.Stage.INDEX, RecoveryState.Stage.TRANSLOG, RecoveryState.Stage.FINALIZE);

        private final Logger logger;

        public BlockingTarget(RecoveryState.Stage stageToBlock, CountDownLatch recoveryBlocked, CountDownLatch releaseRecovery, IndexShard shard, DiscoveryNode sourceNode, PeerRecoveryTargetService.RecoveryListener listener, Logger logger) {
