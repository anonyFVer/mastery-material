...
public class TransportReplicationActionTests extends ESTestCase {
    ...
    public void setUp() throws Exception {
        ...
        transportService = new TransportService(clusterService.getSettings(), transport, threadPool, TransportService.NOOP_TRANSPORT_INTERCEPTOR, null);
    ...
    public void testClosedIndexOnReroute() throws InterruptedException {
        final String index = "test";
        setState(clusterService, new ClusterStateChanges(xContentRegistry()).closeIndices(state(index, true, ShardRoutingState.UNASSIGNED), new CloseIndexRequest(index)));
        logger.debug("--> using initial state:\n{}", clusterService.state());
    ...
    public void testPrimaryPhaseExecutesOrDelegatesRequestToRelocationTarget() throws Exception {
        ...
        action.new AsyncPrimaryAction(request, primaryShard.allocationId().getId(), createTransportChannel(listener), task) {
            ...
            @Override
            protected ReplicationOperation<Request, Request, Action.PrimaryResult> createReplicatedOperation(Request request, ActionListener<Action.PrimaryResult> actionListener, Action.PrimaryShardReference primaryShardReference, boolean executeOnReplicas) {
    ...
    public void testReplicaProxy() throws InterruptedException, ExecutionException {
        ...
        proxy.failShard(replica, randomIntBetween(1, 10), "test", new ElasticsearchException("simulated"), () -> success.set(true), failure::set, ignoredFailure::set);
    ...
    public void testSeqNoIsSetOnPrimary() throws Exception {
        ...
        Action action = new Action(Settings.EMPTY, "testSeqNoIsSetOnPrimary", transportService, clusterService, shardStateAction, threadPool);
        TransportReplicationAction<Request, Request, Response>.PrimaryOperationTransportHandler primaryPhase = action.new PrimaryOperationTransportHandler();
    ...
    class Action extends TransportReplicationAction<Request, Request, Response> {
        ...
        Action(Settings settings, String actionName, TransportService transportService, ClusterService clusterService, ShardStateAction shardStateAction, ThreadPool threadPool) {
