...
public class RelocationIT extends ESIntegTestCase {
    ...
    public void testRelocationWhileRefreshing() throws Exception {
        ...
        nodes[0] = internalCluster().startNode();
        logger.info("--> creating test index ...");
        prepareCreate("test", Settings.builder().put("index.number_of_shards", 1).put("index.number_of_replicas", numberOfReplicas).put("index.refresh_interval", -1).put(IndexService.GLOBAL_CHECKPOINT_SYNC_INTERVAL_SETTING.getKey(), "100ms")).get();
    ...
    public void testCancellationCleansTempFiles() throws Exception {
        ...
        prepareCreate(indexName, Settings.builder().put(IndexMetaData.SETTING_NUMBER_OF_SHARDS, 1).put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, 0)).get();
    ...
    @TestLogging("org.elasticsearch.action.bulk:TRACE," + "org.elasticsearch.action.search:TRACE," + "org.elasticsearch.cluster.service:TRACE," + "org.elasticsearch.index.seqno:TRACE")
    public void testIndexAndRelocateConcurrently() throws ExecutionException, InterruptedException {
        ...
        final Settings.Builder settings = Settings.builder().put("index.routing.allocation.exclude.color", "blue").put(indexSettings()).put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, randomInt(halfNodes - 1)).put(IndexService.GLOBAL_CHECKPOINT_SYNC_INTERVAL_SETTING.getKey(), "100ms");
