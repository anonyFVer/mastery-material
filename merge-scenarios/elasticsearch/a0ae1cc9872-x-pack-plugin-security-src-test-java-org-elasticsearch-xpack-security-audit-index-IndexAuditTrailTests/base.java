...
public class IndexAuditTrailTests extends SecurityIntegTestCase {
    ...
    public Settings nodeSettings(int nodeOrdinal) {
        ...
        return Settings.builder().put(super.nodeSettings(nodeOrdinal)).put("xpack.security.audit.index.settings.index.number_of_shards", numShards).put("xpack.security.audit.index.settings.index.number_of_replicas", numReplicas).build();
    }
    ...
    public void initializeRemoteClusterIfNecessary() throws Exception {
        ...
        SecuritySettingsSource cluster2SettingsSource = new SecuritySettingsSource(useSSL, createTempDir(), Scope.SUITE) {

            @Override
            public Settings nodeSettings(int nodeOrdinal) {
                Settings.Builder builder = Settings.builder().put(super.nodeSettings(nodeOrdinal)).put("xpack.security.audit.index.settings.index.number_of_shards", numShards).put("xpack.security.audit.index.settings.index.number_of_replicas", numReplicas).put(XPackSettings.SECURITY_ENABLED.getKey(), useSecurity);
                String transport = builder.get(NetworkModule.TRANSPORT_TYPE_KEY);
                if (useSecurity == false && (transport == null || SecurityField.NAME4.equals(transport) || SecurityField.NIO.equals(transport))) {
            ...
            public Settings transportClientSettings() {
                if (useSecurity) {
                    ...
                    Settings.Builder builder = Settings.builder().put(XPackSettings.SECURITY_ENABLED.getKey(), false).put(super.transportClientSettings());
        ...
        remoteCluster = new InternalTestCluster(randomLong(), createTempDir(), false, true, numNodes, numNodes, cluster2Name, cluster2SettingsSource, 0, SECOND_CLUSTER_NODE_PREFIX, mockPlugins, useSecurity ? getClientWrapper() : Function.identity());
        ...
        Settings.Builder builder = Settings.builder().put("xpack.security.audit.index.client." + XPackSettings.SECURITY_ENABLED.getKey(), useSecurity).put(remoteSettings(NetworkAddress.format(inet.address().getAddress()), inet.address().getPort(), cluster2Name)).put("xpack.security.audit.index.client.xpack.security.user", SecuritySettingsSource.TEST_USER_NAME + ":" + SecuritySettingsSourceField.TEST_PASSWORD);
        if (remoteUseSSL) {
    ...
    private Settings commonSettings(IndexNameResolver.Rollover rollover) {
        return Settings.builder().put("xpack.security.audit.enabled", true).put("xpack.security.audit.outputs", "index, logfile").put("xpack.security.audit.index.bulk_size", 1).put("xpack.security.audit.index.flush_interval", "1ms").put("xpack.security.audit.index.rollover", rollover.name().toLowerCase(Locale.ENGLISH)).put("xpack.security.audit.index.settings.index.number_of_shards", numShards).put("xpack.security.audit.index.settings.index.number_of_replicas", numReplicas).build();
    }

    static Settings remoteSettings(String address, int port, String clusterName) {
        return Settings.builder().put("xpack.security.audit.index.client.hosts", address + ":" + port).put("xpack.security.audit.index.client.cluster.name", clusterName).build();
    }
    ...
    public void testProcessorsSetting() {
        ...
        final Settings.Builder additionalSettingsBuilder = Settings.builder().put("xpack.security.audit.index.client.cluster.name", "remote").put("xpack.security.audit.index.client.hosts", "localhost:9300");
        if (explicitProcessors) {
        ...
        if (processors == clientProcessors || explicitClientProcessors == false) {
            ...
            assertThat(e, hasToString(containsString("explicit processor setting [" + clientProcessors + "]" + " for audit trail remote client does not match inherited processor setting [" + processors + "]")));
        }
    }
    ...
    public void testSystemAccessGranted() throws Exception {
        ...
        TransportMessage message = randomBoolean() ? new RemoteHostMockMessage() : new LocalHostMockMessage();
        String role = randomAlphaOfLengthBetween(1, 6);
        auditor.accessGranted(randomAlphaOfLength(8), createAuthentication(SystemUser.INSTANCE), "internal:_action", message, new String[] { role });
        SearchHit hit = getIndexedAuditMessage(enqueuedMessage.get());
        assertAuditMessage(hit, "transport", "access_granted");
        Map<String, Object> sourceMap = hit.getSourceAsMap();
        assertEquals("transport", sourceMap.get("origin_type"));
    ...
    private SearchHit getIndexedAuditMessage(Message message) throws InterruptedException {
        ...
        final boolean found = awaitBusy(() -> {
            try {
                SearchResponse searchResponse = getClient().prepareSearch(indexName).setTypes(IndexAuditTrail.DOC_TYPE).get();
    ...
    public ClusterHealthStatus ensureYellowAndNoInitializingShards(String... indices) {
        ...
        ClusterHealthResponse actionGet = getClient().admin().cluster().health(Requests.clusterHealthRequest(indices).waitForNoRelocatingShards(true).waitForYellowStatus().waitForEvents(Priority.LANGUID).waitForNoInitializingShards(true)).actionGet();
