...
public class Watcher implements ActionPlugin, ScriptPlugin {
    ...
    public Collection<Object> createComponents(Clock clock, ScriptService scriptService, InternalClient internalClient, SearchRequestParsers searchRequestParsers, XPackLicenseState licenseState, HttpClient httpClient, HttpRequestTemplate.Parser httpTemplateParser, ThreadPool threadPool, ClusterService clusterService, CryptoService cryptoService, Collection<Object> components) {
        if (enabled == false) {
        ...
        transformFactories.put(SearchTransform.TYPE, new SearchTransformFactory(settings, internalClient, searchRequestParsers, scriptService));
        ...
        final WatcherSearchTemplateService watcherSearchTemplateService = new WatcherSearchTemplateService(settings, scriptService, searchRequestParsers);
        ...
        final ExecutionService executionService = new ExecutionService(settings, historyStore, triggeredWatchStore, watchExecutor, watchLockService, clock, threadPool, watchParser, watcherClientProxy);
        ...
        final WatcherService watcherService = new WatcherService(settings, triggerService, executionService, watchLockService, watcherIndexTemplateRegistry, watchParser, watcherClientProxy);
