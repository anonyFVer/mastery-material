...
public abstract class IndexShardTestCase extends ESTestCase {
    ...
    protected IndexShard newShard(final ShardRouting shardRouting, final Settings settings, final EngineFactory engineFactory, final IndexingOperationListener... listeners) throws IOException {
        ...
        Settings indexSettings = Settings.builder().put(IndexMetaData.SETTING_VERSION_CREATED, Version.CURRENT).put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, 0).put(IndexMetaData.SETTING_NUMBER_OF_SHARDS, 1).put(settings).build();
    ...
    protected Engine.IndexResult indexDoc(IndexShard shard, String type, String id, String source, XContentType xContentType, String routing) throws IOException {
        ...
        if (shard.routingEntry().primary()) {
            ...
            shard.updateLocalCheckpointForShard(shard.routingEntry().allocationId().getId(), shard.getEngine().getLocalCheckpointTracker().getCheckpoint());
        } else {
