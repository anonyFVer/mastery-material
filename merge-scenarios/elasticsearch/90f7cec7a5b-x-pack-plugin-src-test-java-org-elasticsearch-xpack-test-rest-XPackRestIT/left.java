...
public class XPackRestIT extends ESClientYamlSuiteTestCase {
    ...
    private void waitForTemplates() throws Exception {
        if (installTemplates()) {
            ...
            templates.addAll(Arrays.asList(AuditorField.NOTIFICATIONS_INDEX, MlMetaIndex.INDEX_NAME, AnomalyDetectorsIndex.jobStateIndexName(), AnomalyDetectorsIndex.jobResultsIndexPrefix()));
            for (String template : templates) {
                awaitCallApi("indices.exists_template", singletonMap("name", template), emptyList(), response -> true, () -> "Exception when waiting for [" + template + "] template to be created");
            }
        }
    }
    ...
    private void waitForWatcher() throws Exception {
        if (isWatcherTest()) {
            ...
            boolean existsWatcherIndex = adminClient().performRequest(new Request("HEAD", ".watches")).getStatusLine().getStatusCode() == 200;
    ...
    private void disableMonitoring() throws Exception {
        if (isMonitoringTest()) {
            ...
            awaitBusy(() -> {
                try {
                    ClientYamlTestResponse response = callApi("xpack.usage", singletonMap("filter_path", "monitoring.enabled_exporters"), emptyList(), getApiCallHeaders());
                    @SuppressWarnings("unchecked")
    ...
    private void awaitCallApi(String apiName, Map<String, String> params, List<Map<String, Object>> bodies, CheckedFunction<ClientYamlTestResponse, Boolean, IOException> success, Supplier<String> error) throws Exception {
    ...
    private ClientYamlTestResponse callApi(String apiName, Map<String, String> params, List<Map<String, Object>> bodies, Map<String, String> headers) throws IOException {
    ...
    protected boolean isILMTest() {
        String testName = getTestName();
        return testName != null && (testName.contains("=ilm/") || testName.contains("=ilm\\")) || (testName.contains("/ilm/") || testName.contains("\\ilm\\"));
    }
