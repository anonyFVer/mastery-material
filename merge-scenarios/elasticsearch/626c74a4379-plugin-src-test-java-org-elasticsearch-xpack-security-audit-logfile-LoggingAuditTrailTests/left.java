...
public class LoggingAuditTrailTests extends ESTestCase {
    ...
    public void testAccessGrantedInternalSystemAction() throws Exception {
        ...
        auditTrail.accessGranted(SystemUser.INSTANCE, "internal:_action", message, null);
        if (message instanceof IndicesRequest) {
            assertMsg(logger, Level.INFO, prefix + "[transport] [access_granted]\t" + origins + ", principal=[" + SystemUser.INSTANCE.principal() + "], action=[internal:_action], indices=[" + indices(message) + "], request=[MockIndicesRequest]");
        } else {
            assertMsg(logger, Level.INFO, prefix + "[transport] [access_granted]\t" + origins + ", principal=[" + SystemUser.INSTANCE.principal() + "], action=[internal:_action], request=[MockMessage]");
        }
    }
    ...
    public void testRunAsGranted() throws Exception {
        ...
        assertMsg(logger, Level.INFO, prefix + "[transport] [run_as_granted]\t" + origins + ", principal=[_username], run_as_principal=[running as], action=[_action], request=[MockMessage]");
        CapturingLogger.output(logger.getName(), Level.INFO).clear();
    ...
    public void testRunAsDenied() throws Exception {
        ...
        assertMsg(logger, Level.INFO, prefix + "[transport] [run_as_denied]\t" + origins + ", principal=[_username], run_as_principal=[running as], action=[_action], request=[MockMessage]");
        CapturingLogger.output(logger.getName(), Level.INFO).clear();
