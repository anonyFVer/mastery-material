...
public class TransportReplicationActionTests extends ESTestCase {
    ...
    public void testBlocks() throws ExecutionException, InterruptedException {
        ...
        ClusterBlocks.Builder block = ClusterBlocks.builder().addGlobalBlock(new ClusterBlock(1, "non retryable", false, true, false, RestStatus.SERVICE_UNAVAILABLE, ClusterBlockLevel.ALL));
        setState(clusterService, ClusterState.builder(clusterService.state()).blocks(block));
        TestAction.ReroutePhase reroutePhase = action.new ReroutePhase(task, request, listener);
        reroutePhase.run();
        assertListenerThrows("primary phase should fail operation", listener, ClusterBlockException.class);
        assertPhase(task, "failed");
        block = ClusterBlocks.builder().addGlobalBlock(new ClusterBlock(1, "retryable", true, true, false, RestStatus.SERVICE_UNAVAILABLE, ClusterBlockLevel.ALL));
        setState(clusterService, ClusterState.builder(clusterService.state()).blocks(block));
        ...
        block = ClusterBlocks.builder().addGlobalBlock(new ClusterBlock(1, "non retryable", false, true, false, RestStatus.SERVICE_UNAVAILABLE, ClusterBlockLevel.ALL));
        setState(clusterService, ClusterState.builder(clusterService.state()).blocks(block));
        ...
        action = new TestAction(Settings.EMPTY, "internal:testActionWithNoBlocks", transportService, clusterService, shardStateAction, threadPool) {
    ...
    private Exception randomRetryPrimaryException(ShardId shardId) {
        return randomFrom(new ShardNotFoundException(shardId), new IndexNotFoundException(shardId.getIndex()), new IndexShardClosedException(shardId), new AlreadyClosedException(shardId + " primary is closed"), new ReplicationOperation.RetryOnPrimaryException(shardId, "hello"));
    }
    ...
    public void testPrimaryPhaseExecutesOrDelegatesRequestToRelocationTarget() throws Exception {
        ...
        if (executeOnPrimary) {
            ...
            assertThat("primary term not properly set on primary delegation", ((TransportReplicationAction.ConcreteShardRequest<Request>) requests.get(0).request).getPrimaryTerm(), equalTo(primaryTerm));
    ...
    public void testPrimaryPhaseExecutesDelegatedRequestOnRelocationTarget() throws Exception {
        ...
        ReplicationTask task = maybeTask();
        AtomicBoolean executed = new AtomicBoolean();
        action.new AsyncPrimaryAction(request, primaryShard.allocationId().getRelocationId(), primaryTerm, createTransportChannel(listener), task) {
    ...
    public void testReplicaProxy() throws InterruptedException, ExecutionException {
        ...
        proxy.performOn(TestShardRouting.newShardRouting(shardId, "NOT THERE", routingState == ShardRoutingState.RELOCATING ? state.nodes().iterator().next().getId() : null, false, routingState), new Request(), randomNonNegativeLong(), randomNonNegativeLong(), listener);
        assertTrue(listener.isDone());
    ...
    public void testSeqNoIsSetOnPrimary() throws Exception {
        ...
        Set<String> inSyncIds = randomBoolean() ? Collections.singleton(routingEntry.allocationId().getId()) : clusterService.state().metaData().index(index).inSyncAllocationIds(0);
        when(shard.getReplicationGroup()).thenReturn(new ReplicationGroup(shardRoutingTable, inSyncIds, shardRoutingTable.getAllAllocationIds()));
        doAnswer(invocation -> {
            ((ActionListener<Releasable>) invocation.getArguments()[0]).onResponse(() -> {
            });
            return null;
        }).when(shard).acquirePrimaryOperationPermit(any(), anyString(), anyObject());
        TestAction action = new TestAction(Settings.EMPTY, "internal:testSeqNoIsSetOnPrimary", transportService, clusterService, shardStateAction, threadPool) {
        ...
        TransportReplicationAction<Request, Request, TestResponse>.PrimaryOperationTransportHandler primaryPhase = action.new PrimaryOperationTransportHandler();
    ...
    public void testReplicasCounter() throws Exception {
        ...
        try {
            replicaOperationTransportHandler.messageReceived(new TransportReplicationAction.ConcreteReplicaRequest<>(new Request().setShardId(shardId), replicaRouting.allocationId().getId(), randomNonNegativeLong(), randomNonNegativeLong(), randomNonNegativeLong()), createTransportChannel(new PlainActionFuture<>()), task);
    ...
    public void testPrimaryActionRejectsWrongAidOrWrongTerm() throws Exception {
        ...
        try {
            ...
            if (wrongAllocationId) {
                assertThat(throwable.getMessage(), containsString("expected aID [_not_a_valid_aid_] but found [" + primary.allocationId().getId() + "]"));
            } else {
                assertThat(throwable.getMessage(), containsString("expected aID [" + primary.allocationId().getId() + "] with term [" + requestTerm + "] but found [" + primaryTerm + "]"));
            }
        }
    }
    ...
    public void testRetryOnReplica() throws Exception {
        ...
        replicaOperationTransportHandler.messageReceived(new TransportReplicationAction.ConcreteReplicaRequest<>(request, replica.allocationId().getId(), primaryTerm, checkpoint, maxSeqNoOfUpdatesOrDeletes), createTransportChannel(listener), task);
        if (listener.isDone()) {
            listener.get();
    ...
    public void testRetryOnReplicaWithRealTransport() throws Exception {
        ...
        final Transport transport = new MockTcpTransport(Settings.EMPTY, threadPool, BigArrays.NON_RECYCLING_INSTANCE, new NoneCircuitBreakerService(), namedWriteableRegistry, new NetworkService(Collections.emptyList()));
        ...
        replicaOperationTransportHandler.messageReceived(new TransportReplicationAction.ConcreteReplicaRequest<>(request, replica.allocationId().getId(), primaryTerm, checkpoint, maxSeqNoOfUpdates), createTransportChannel(listener), task);
        if (listener.isDone()) {
            listener.get();
            fail("listener shouldn't be done");
        }
    ...
    private void assertConcreteShardRequest(TransportRequest capturedRequest, Request expectedRequest, AllocationId expectedAllocationId) {
        final TransportReplicationAction.ConcreteShardRequest<?> concreteShardRequest = (TransportReplicationAction.ConcreteShardRequest<?>) capturedRequest;
    ...
    private class TestAction extends TransportReplicationAction<Request, Request, TestResponse> {
        ...
        TestAction(Settings settings, String actionName, TransportService transportService, ClusterService clusterService, ShardStateAction shardStateAction, ThreadPool threadPool) {
            super(settings, actionName, transportService, clusterService, mockIndicesService(clusterService), threadPool, shardStateAction, new ActionFilters(new HashSet<>()), new IndexNameExpressionResolver(), Request::new, Request::new, ThreadPool.Names.SAME);
        }
