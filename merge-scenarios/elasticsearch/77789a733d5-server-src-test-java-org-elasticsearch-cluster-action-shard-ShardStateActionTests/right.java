...
public class ShardStateActionTests extends ESTestCase {
    ...
    private static class TestShardStateAction extends ShardStateAction {
        ...
        TestShardStateAction(ClusterService clusterService, TransportService transportService, AllocationService allocationService, RoutingService routingService) {
    ...
    public void testNoLongerPrimaryShardException() throws InterruptedException {
        ...
        shardStateAction.remoteShardFailed(failedShard.shardId(), failedShard.allocationId().getId(), primaryTerm + 1, randomBoolean(), "test", getSimulatedFailure(), new ShardStateAction.Listener() {
    ...
    public void testCacheRemoteShardFailed() throws Exception {
        ...
        for (int i = 0; i < numListeners; i++) {
            shardStateAction.remoteShardFailed(failedShard.shardId(), failedShard.allocationId().getId(), primaryTerm, markAsStale, "test", getSimulatedFailure(), new ShardStateAction.Listener() {
    ...
    public void testRemoteShardFailedConcurrently() throws Exception {
        ...
        Phaser barrier = new Phaser(clientThreads.length + 2);
        ...
        for (int t = 0; t < clientThreads.length; t++) {
            clientThreads[t] = new Thread(() -> {
                ...
                for (int i = 0; i < iterationsPerThread; i++) {
                    ...
                    shardStateAction.remoteShardFailed(failedShard.shardId(), failedShard.allocationId().getId(), randomLongBetween(1, Long.MAX_VALUE), randomBoolean(), "test", getSimulatedFailure(), new ShardStateAction.Listener() {
    ...
    public void testShardEntryBWCSerialize() throws Exception {
        final Version bwcVersion = randomValueOtherThanMany(version -> version.onOrAfter(Version.V_6_3_0), () -> VersionUtils.randomVersion(random()));
