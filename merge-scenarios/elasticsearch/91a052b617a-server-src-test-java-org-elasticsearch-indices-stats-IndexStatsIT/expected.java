...
public class IndexStatsIT extends ESIntegTestCase {
    ...
    protected Settings nodeSettings(int nodeOrdinal) {
        return Settings.builder().put(super.nodeSettings(nodeOrdinal)).put(IndicesService.INDICES_CACHE_CLEAN_INTERVAL_SETTING.getKey(), "1ms").put(IndicesQueryCache.INDICES_QUERIES_CACHE_ALL_SEGMENTS_SETTING.getKey(), true).build();
    }

    @Override
    public Settings indexSettings() {
        return Settings.builder().put(super.indexSettings()).put(IndexModule.INDEX_QUERY_CACHE_EVERYTHING_SETTING.getKey(), true).put(IndexModule.INDEX_QUERY_CACHE_ENABLED_SETTING.getKey(), true).put(IndexSettings.INDEX_SOFT_DELETES_RETENTION_OPERATIONS_SETTING.getKey(), 0).build();
    }
    ...
    public void testSimpleStats() throws Exception {
        ...
        try {
            client().prepareIndex("test1", "type", Integer.toString(1)).setSource("field", "value").setVersion(1).setVersionType(VersionType.EXTERNAL).execute().actionGet();
    ...
    public void testSegmentsStats() {
        assertAcked(prepareCreate("test_index").setSettings(Settings.builder().put(SETTING_NUMBER_OF_REPLICAS, between(0, 1))));
        ensureGreen();
    ...
    public void testFlagOrdinalOrder() {
        Flag[] flags = new Flag[] { Flag.Store, Flag.Indexing, Flag.Get, Flag.Search, Flag.Merge, Flag.Flush, Flag.Refresh, Flag.QueryCache, Flag.FieldData, Flag.Docs, Flag.Warmer, Flag.Completion, Flag.Segments, Flag.Translog, Flag.RequestCache, Flag.Recovery };
    ...
    public void testFieldDataFieldsParam() throws Exception {
        assertAcked(client().admin().indices().prepareCreate("test1").setSettings(Settings.builder().put("index.version.created", Version.V_6_0_0.id)).addMapping("_doc", "bar", "type=text,fielddata=true", "baz", "type=text,fielddata=true").get());
        ensureGreen();
    ...
    public void testFilterCacheStats() throws Exception {
        ...
        indexRandom(false, true, client().prepareIndex("index", "type", "1").setSource("foo", "bar"), client().prepareIndex("index", "type", "2").setSource("foo", "baz"));
    ...
    public void testConcurrentIndexingAndStatsRequests() throws BrokenBarrierException, InterruptedException, ExecutionException {
        ...
        final AtomicReference<List<DefaultShardOperationFailedException>> shardFailures = new AtomicReference<>(new CopyOnWriteArrayList<>());
