...
public class GatewayIndexStateIT extends ESIntegTestCase {
    ...
    public void testMappingMetaDataParsed() throws Exception {
        ...
        MappingMetaData mappingMd = client().admin().cluster().prepareState().execute().actionGet().getState().metaData().index("test").getMappings().get("type1");
        ...
        ensureYellow();
        logger.info("--> verify meta _routing required exists");
        mappingMd = client().admin().cluster().prepareState().execute().actionGet().getState().metaData().index("test").getMappings().get("type1");
        assertThat(mappingMd.routing().required(), equalTo(true));
    }
    ...
    public void testRecoverMissingAnalyzer() throws Exception {
        ...
        ClusterState state = client().admin().cluster().prepareState().get().getState();
        final IndexMetaData metaData = state.getMetaData().index("test");
        final IndexMetaData brokenMeta = IndexMetaData.builder(metaData).settings(metaData.getSettings().filter((s) -> "index.analysis.analyzer.test.tokenizer".equals(s) == false)).build();
        internalCluster().fullRestart(new RestartCallback() {
    ...
    public void testArchiveBrokenClusterSettings() throws Exception {
        ...
        final MetaData brokenMeta = MetaData.builder(metaData).persistentSettings(Settings.builder().put(metaData.persistentSettings()).put("this.is.unknown", true).put(ElectMasterService.DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING.getKey(), "broken").build()).build();
        internalCluster().fullRestart(new RestartCallback() {
