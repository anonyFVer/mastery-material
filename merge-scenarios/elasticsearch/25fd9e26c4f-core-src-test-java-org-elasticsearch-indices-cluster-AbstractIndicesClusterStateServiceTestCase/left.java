...
public abstract class AbstractIndicesClusterStateServiceTestCase extends ESTestCase {
    ...
    public static void assertClusterStateMatchesNodeState(ClusterState state, IndicesClusterStateService indicesClusterStateService) {
        AllocatedIndices<? extends Shard, ? extends AllocatedIndex<? extends Shard>> indicesService = indicesClusterStateService.indicesService;
            ...
            for (ShardRouting shardRouting : localRoutingNode) {
                ...
                if (shard != null) {
                    ...
                    if (failedShard == null) {
                        ...
                        final Set<String> initializingIds = shardRoutingTable.getAllInitializingShards().stream().map(r -> r.allocationId().getId()).collect(Collectors.toSet());
                        final Set<String> activeIds = shardRoutingTable.activeShards().stream().map(r -> r.allocationId().getId()).collect(Collectors.toSet());
    ...
    protected class MockIndicesService implements AllocatedIndices<MockIndexShard, MockIndexService> {
        ...
        @Override
        public synchronized MockIndexService createIndex(NodeServicesProvider nodeServicesProvider, IndexMetaData indexMetaData, List<IndexEventListener> buildInIndexListener, Consumer<ShardId> globalCheckpointSyncer) throws IOException {
        ...
        @Override
        public void processPendingDeletes(Index index, IndexSettings indexSettings, TimeValue timeValue) throws IOException, InterruptedException {
        }
