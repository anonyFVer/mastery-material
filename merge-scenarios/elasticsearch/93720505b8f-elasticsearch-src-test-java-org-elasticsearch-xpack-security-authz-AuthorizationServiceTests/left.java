...
public class AuthorizationServiceTests extends ESTestCase {
    ...
    public void setup() {
        ...
        authorizationService = new AuthorizationService(Settings.EMPTY, rolesStore, clusterService, auditTrail, new DefaultAuthenticationFailureHandler(), threadPool, new AnonymousUser(Settings.EMPTY));
    }
    ...
    public void testRunAsRequestRunningAsUnAllowedUser() {
        ...
        assertThat(user.runAs(), is(notNullValue()));
        roleMap.put("can run as", Role.builder("can run as").runAs(new GeneralPrivilege("", "not the right user")).add(IndexPrivilege.ALL, "a").build());
        assertThrowsAuthorizationExceptionRunAs(() -> authorize(createAuthentication(user), "indices:a", request), "indices:a", "test user", "run as me");
        verify(auditTrail).runAsDenied(user, "indices:a", request);
        verifyNoMoreInteractions(auditTrail);
    }
    ...
    public void testRunAsRequestWithRunAsUserWithoutPermission() {
        ...
        assertThat(user.runAs(), is(notNullValue()));
        roleMap.put("can run as", Role.builder("can run as").runAs(new GeneralPrivilege("", "run as me")).add(IndexPrivilege.ALL, "a").build());
        if (randomBoolean()) {
            ...
            roleMap.put("b", Role.builder("b").add(IndexPrivilege.ALL, "b").build());
        } else {
            mockEmptyMetaData();
        }
        assertThrowsAuthorizationExceptionRunAs(() -> authorize(createAuthentication(user), "indices:a", request), "indices:a", "test user", "run as me");
    ...
    public void testNonXPackUserCannotExecuteOperationAgainstSecurityIndex() {
        ...
        roleMap.put("all_access", Role.builder("all_access").add(IndexPrivilege.ALL, "*").cluster(ClusterPrivilege.ALL).build());
        ClusterState state = mock(ClusterState.class);
        when(clusterService.state()).thenReturn(state);
        when(state.metaData()).thenReturn(MetaData.builder().put(new IndexMetaData.Builder(SecurityTemplateService.SECURITY_INDEX_NAME).settings(Settings.builder().put("index.version.created", Version.CURRENT).build()).numberOfShards(1).numberOfReplicas(0).build(), true).build());
        ...
        requests.add(new Tuple<>(TermVectorsAction.NAME, new TermVectorsRequest(SecurityTemplateService.SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(GetAction.NAME, new GetRequest(SecurityTemplateService.SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(TermVectorsAction.NAME, new TermVectorsRequest(SecurityTemplateService.SECURITY_INDEX_NAME, "type", "id")));
        requests.add(new Tuple<>(IndicesAliasesAction.NAME, new IndicesAliasesRequest().addAliasAction(AliasActions.add().alias("security_alias").index(SecurityTemplateService.SECURITY_INDEX_NAME))));
        requests.add(new Tuple<>(UpdateSettingsAction.NAME, new UpdateSettingsRequest().indices(SecurityTemplateService.SECURITY_INDEX_NAME)));
    ...
    public void testGrantedNonXPackUserCanExecuteMonitoringOperationsAgainstSecurityIndex() {
        ...
        requests.add(new Tuple<>(IndicesSegmentsAction.NAME, new IndicesSegmentsRequest().indices(SecurityTemplateService.SECURITY_INDEX_NAME)));
        requests.add(new Tuple<>(GetSettingsAction.NAME, new GetSettingsRequest().indices(SecurityTemplateService.SECURITY_INDEX_NAME)));
        requests.add(new Tuple<>(IndicesShardStoresAction.NAME, new IndicesShardStoresRequest().indices(SecurityTemplateService.SECURITY_INDEX_NAME)));
        requests.add(new Tuple<>(UpgradeStatusAction.NAME, new UpgradeStatusRequest().indices(SecurityTemplateService.SECURITY_INDEX_NAME)));
    ...
    public void testXPackUserAndSuperusersCanExecuteOperationAgainstSecurityIndex() {
        ...
        for (User user : Arrays.asList(XPackUser.INSTANCE, superuser)) {
            ...
            requests.add(new Tuple<>(ClusterHealthAction.NAME, new ClusterHealthRequest(SecurityTemplateService.SECURITY_INDEX_NAME, "foo", "bar")));
            for (Tuple<String, TransportRequest> requestTuple : requests) {
                String action = requestTuple.v1();
                TransportRequest request = requestTuple.v2();
                authorize(createAuthentication(user), action, request);
    ...
    public void testAnonymousRolesAreAppliedToOtherUsers() {
        ...
        final AnonymousUser anonymousUser = new AnonymousUser(settings);
        authorizationService = new AuthorizationService(settings, rolesStore, clusterService, auditTrail, new DefaultAuthenticationFailureHandler(), threadPool, anonymousUser);
        roleMap.put("anonymous_user_role", Role.builder("anonymous_user_role").cluster(ClusterPrivilege.ALL).add(IndexPrivilege.ALL, "a").build());
        mockEmptyMetaData();
    ...
    public void testDefaultRoleUserWithSomeRole() {
        roleMap.put("role", Role.builder("role").cluster(ClusterPrivilege.ALL).add(IndexPrivilege.ALL, "a").build());
        PlainActionFuture<Collection<Role>> rolesFuture = new PlainActionFuture<>();
    ...
    public void testCompositeActionsIndicesAreNotChecked() {
        ...
        roleMap.put("role", Role.builder("role").add(IndexPrivilege.ALL, randomBoolean() ? "a" : "index").build());
        authorize(createAuthentication(user), action, request);
        verify(auditTrail).accessGranted(user, action, request);
        verifyNoMoreInteractions(auditTrail);
    }
    ...
    public void testPermissionIncludesAnonymousUserPermissions() {
        ...
        final MetaData metaData = MetaData.builder().put(new IndexMetaData.Builder("a").settings(Settings.builder().put("index.version.created", Version.CURRENT).build()).numberOfShards(1).numberOfReplicas(0).build(), true).build();
        ...
        if (roleExists) {
            ...
            Map<String, IndexAccessControl> authzMap = globalPermission.indices().authorize(SearchAction.NAME, Collections.singleton("a"), metaData);
