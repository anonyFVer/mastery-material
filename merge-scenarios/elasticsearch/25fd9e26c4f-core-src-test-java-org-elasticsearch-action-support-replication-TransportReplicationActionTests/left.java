...
public class TransportReplicationActionTests extends ESTestCase {
    ...
    public void testReplicaProxy() throws InterruptedException, ExecutionException {
        ...
        proxy.failShard(replica, shardRoutings.primaryShard(), "test", new ElasticsearchException("simulated"), () -> success.set(true), failure::set, ignoredFailure::set);
    ...
    class Action extends TransportReplicationAction<Request, Request, Response> {
        ...
        Action(Settings settings, String actionName, TransportService transportService, ClusterService clusterService, ThreadPool threadPool) {
            super(settings, actionName, transportService, clusterService, null, threadPool, new ShardStateAction(settings, clusterService, transportService, null, null, threadPool), new ActionFilters(new HashSet<>()), new IndexNameExpressionResolver(Settings.EMPTY), Request::new, Request::new, ThreadPool.Names.SAME);
        }
        ...
        protected void acquirePrimaryShardReference(ShardId shardId, ActionListener<PrimaryShardReference> onReferenceAcquired) {
            ...
            PrimaryShardReference primaryShardReference = new PrimaryShardReference(null, null) {
                ...
                public ShardRouting routingEntry() {
                    ShardRouting shardRouting = clusterService.state().getRoutingTable().shardRoutingTable(shardId).primaryShard();
