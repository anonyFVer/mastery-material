...
public class ClusterService extends AbstractLifecycleComponent {
    ...
    private void updateState(UnaryOperator<ClusterServiceState> updateFunction) {
        this.state.getAndUpdate(oldClusterServiceState -> {
            ...
            assert validStateTransition(oldClusterServiceState, newClusterServiceState) : "Invalid cluster service state transition from " + oldClusterServiceState + " to " + newClusterServiceState;
    ...
    protected synchronized void doStart() {
        ...
        updateState(css -> new ClusterServiceState(ClusterState.builder(css.getClusterState()).blocks(initialBlocks).build(), css.getClusterStateStatus()));
    ...
    public <T> void submitStateUpdateTasks(final String source, final Map<T, ClusterStateTaskListener> tasks, final ClusterStateTaskConfig config, final ClusterStateTaskExecutor<T> executor) {
        ...
        try {
            ...
            final List<UpdateTask<T>> updateTasks = tasksIdentity.entrySet().stream().map(entry -> new UpdateTask<>(source, entry.getKey(), config.priority(), executor, safe(entry.getValue(), logger))).collect(Collectors.toList());
    ...
    <T> void runTasksForExecutor(ClusterStateTaskExecutor<T> executor) {
        ...
        if (assertsEnabled) {
            for (UpdateTask<T> updateTask : toExecute) {
                assert batchResult.executionResults.containsKey(updateTask.task) : "missing task result for " + updateTask;
            }
        }
        ...
        for (UpdateTask<T> updateTask : toExecute) {
            ...
            executionResult.handle(() -> proccessedListeners.add(updateTask), ex -> {
                logger.debug((Supplier<?>) () -> new ParameterizedMessage("cluster state update task {} failed", updateTask), ex);
