...
public class SharedClusterSnapshotRestoreIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testSnapshotSucceedsAfterSnapshotFailure() throws Exception {
        ...
        final Client client = client();
        assertAcked(client.admin().cluster().preparePutRepository("test-repo").setType("mock").setVerify(false).setSettings(Settings.builder().put("location", repoPath).put("random_control_io_exception_rate", randomIntBetween(5, 20) / 100f).put("atomic_move", false).put("random", randomAlphaOfLength(10))));
        logger.info("--> indexing some data");
