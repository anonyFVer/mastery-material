...
public class PeerRecoveryTargetService extends AbstractComponent implements IndexEventListener {
    ...
    private void waitForClusterState(long clusterStateVersion) {
        ...
        if (clusterState.getVersion() >= clusterStateVersion) {
            ...
            try {
                future.get();
                logger.trace("successfully waited for cluster state with version {} (current: {})", clusterStateVersion, observer.observedState().getClusterState().getVersion());
            } catch (Exception e) {
                logger.debug((Supplier<?>) () -> new ParameterizedMessage("failed waiting for cluster state with version {} (current: {})", clusterStateVersion, observer.observedState().getClusterState().getVersion()), e);
