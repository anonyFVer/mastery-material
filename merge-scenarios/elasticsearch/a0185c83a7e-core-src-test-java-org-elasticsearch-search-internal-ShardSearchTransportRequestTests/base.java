...
public class ShardSearchTransportRequestTests extends AbstractSearchTestCase {
    ...
    private IndexMetaData baseMetaData = IndexMetaData.builder("test").settings(Settings.builder().put(IndexMetaData.SETTING_VERSION_CREATED, Version.CURRENT).build()).numberOfShards(1).numberOfReplicas(1).build();
    ...
    private ShardSearchTransportRequest createShardSearchTransportRequest() throws IOException {
        ...
        return new ShardSearchTransportRequest(searchRequest, shardId, randomIntBetween(1, 100), filteringAliases, Math.abs(randomLong()));
    }
    ...
    public void testFilteringAliases() throws Exception {
        ...
        assertEquals(aliasFilter(indexMetaData, "cats", "dogs"), QueryBuilders.boolQuery().should(QueryBuilders.termQuery("animal", "cat")).should(QueryBuilders.termQuery("animal", "dog")));
        ...
        assertEquals(aliasFilter(indexMetaData, "dogs", "cats"), QueryBuilders.boolQuery().should(QueryBuilders.termQuery("animal", "canine")).should(QueryBuilders.termQuery("animal", "feline")));
    }
    ...
    public QueryBuilder aliasFilter(IndexMetaData indexMetaData, String... aliasNames) {
        Function<XContentParser, QueryParseContext> contextFactory = (p) -> new QueryParseContext(queriesRegistry, p, new ParseFieldMatcher(Settings.EMPTY));
    ...
    public void testSerialize50Request() throws IOException {
        ...
        try (StreamInput in = new NamedWriteableAwareStreamInput(requestBytes.streamInput(), namedWriteableRegistry)) {
            ...
            QueryShardContext context = new QueryShardContext(0, indexSettings, null, null, null, null, null, queriesRegistry, null, null, null, () -> nowInMillis);
