...
public class MonitoringIT extends ESSingleNodeTestCase {
    ...
    public void testMonitoringBulk() throws Exception {
        whenExportersAreReady(() -> {
            ...
            final MonitoringBulkResponse bulkResponse = new MonitoringBulkRequestBuilder(client()).add(system, null, new BytesArray(createBulkEntity().getBytes("UTF-8")), XContentType.JSON, System.currentTimeMillis(), interval.millis()).get();
            ...
            assertBusy(() -> {
                ...
                final SearchResponse response = client().prepareSearch(".monitoring-" + system.getSystem() + "-" + TEMPLATE_VERSION + "-*").get();
                assertThat("No monitoring documents yet", response.getHits().getTotalHits().value, equalTo(3L));
                final List<Map<String, Object>> sources = Arrays.stream(response.getHits().getHits()).map(SearchHit::getSourceAsMap).collect(Collectors.toList());
            ...
            assertThat("Monitoring documents must have the same timestamp", Arrays.stream(hits.getHits()).map(hit -> extractValue("timestamp", hit.getSourceAsMap())).distinct().count(), equalTo(1L));
            assertThat("Monitoring documents must have the same source_node timestamp", Arrays.stream(hits.getHits()).map(hit -> extractValue("source_node.timestamp", hit.getSourceAsMap())).distinct().count(), equalTo(1L));
    ...
    public void testMonitoringService() throws Exception {
        ...
        assertThat(client().prepareIndex(indexName, "doc", "0").setRefreshPolicy("true").setSource("{\"field\":\"value\"}", XContentType.JSON).get().status(), is(RestStatus.CREATED));
        ...
        whenExportersAreReady(() -> {
            ...
            assertBusy(() -> {
                final SearchResponse response = client().prepareSearch(".monitoring-es-*").setCollapse(new CollapseBuilder("type")).addSort("timestamp", SortOrder.DESC).get();
                assertThat(response.status(), is(RestStatus.OK));
                assertThat("Expecting a minimum number of 6 docs, one per collector", response.getHits().getHits().length, greaterThanOrEqualTo(6));
    ...
    private void assertMonitoringDoc(final Map<String, Object> document, final MonitoredSystem expectedSystem, final String expectedType, final TimeValue interval) {
        ...
        assertThat(index, equalTo(MonitoringTemplateUtils.indexName(DateTimeFormat.forPattern("YYYY.MM.dd").withZoneUTC(), expectedSystem, ISODateTimeFormat.dateTime().parseMillis(timestamp))));
    ...
    @SuppressWarnings("unchecked")
    private void assertClusterStatsMonitoringDoc(final Map<String, Object> document, final boolean apmIndicesExist) {
        final Map<String, Object> source = (Map<String, Object>) document.get("_source");
    ...
    private void assertIndicesStatsMonitoringDoc(final Map<String, Object> document) {
        ...
        IndicesStatsMonitoringDoc.XCONTENT_FILTERS.forEach(filter -> assertThat(filter + " must not be null in the monitoring document", extractValue(filter, source), notNullValue()));
    }
    ...
    private void assertIndexStatsMonitoringDoc(final Map<String, Object> document) {
        ...
        IndexStatsMonitoringDoc.XCONTENT_FILTERS.forEach(filter -> assertThat(filter + " must not be null in the monitoring document", extractValue(filter, source), notNullValue()));
    }
    ...
    public void enableMonitoring() throws Exception {
        ...
        assertBusy(() -> {
            ...
            assertThat("No monitoring documents yet", client().prepareSearch(".monitoring-es-" + TEMPLATE_VERSION + "-*").setSize(0).get().getHits().getTotalHits().value, greaterThan(0L));
        });
    }

    public void disableMonitoring() throws Exception {
        final Settings settings = Settings.builder().putNull("xpack.monitoring.collection.enabled").putNull("xpack.monitoring.exporters._local.enabled").putNull("cluster.metadata.display_name").build();
        assertAcked(client().admin().cluster().prepareUpdateSettings().setTransientSettings(settings));
