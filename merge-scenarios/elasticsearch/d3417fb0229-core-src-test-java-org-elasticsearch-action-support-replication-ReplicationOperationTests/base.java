...
public class ReplicationOperationTests extends ESTestCase {
    ...
    public void testWaitForActiveShards() throws Exception {
        ...
        logger.debug("using active shard count of [{}], assigned shards [{}], total shards [{}]." + " expecting op to [{}]. using state: \n{}", request.waitForActiveShards(), 1 + assignedReplicas, 1 + assignedReplicas + unassignedReplicas, passesActiveShardCheck ? "succeed" : "retry", state.prettyPrint());
    ...
    class TestReplicationOperation extends ReplicationOperation<Request, Request, TestPrimary.Result> {
        ...
        public TestReplicationOperation(Request request, Primary<Request, Request, TestPrimary.Result> primary, ActionListener<TestPrimary.Result> listener, boolean executeOnReplicas, Replicas<Request> replicas, Supplier<ClusterState> clusterStateSupplier, Logger logger, String opType) {
