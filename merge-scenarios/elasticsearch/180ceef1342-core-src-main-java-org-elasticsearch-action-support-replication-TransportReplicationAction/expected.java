...
public abstract class TransportReplicationAction<Request extends ReplicationRequest<Request>, ReplicaRequest extends ReplicationRequest<ReplicaRequest>, Response extends ReplicationResponse> extends TransportAction<Request, Response> {
    ...
    private final class AsyncReplicaAction extends AbstractRunnable implements ActionListener<Releasable> {
        ...
        public void onResponse(Releasable releasable) {
            try {
                ...
                releasable.close();
                final TransportReplicationAction.ReplicaResponse response = new ReplicaResponse(replica.routingEntry().allocationId().getId(), replica.getLocalCheckpoint());
    ...
    class PrimaryShardReference extends ShardReference implements ReplicationOperation.Primary<Request, ReplicaRequest, PrimaryResult<ReplicaRequest, Response>> {
        ...
        public PrimaryResult perform(Request request) throws Exception {
            ...
            if (result.replicaRequest() != null) {
                assert result.finalFailure == null : "a replica request [" + result.replicaRequest() + "] with a primary failure [" + result.finalFailure + "]";
    ...
    final class ReplicasProxy implements ReplicationOperation.Replicas<ReplicaRequest> {
        ...
        public void performOn(ShardRouting replica, ReplicaRequest request, ActionListener<ReplicationOperation.ReplicaResponse> listener) {
            ...
            final ConcreteShardRequest<ReplicaRequest> concreteShardRequest = new ConcreteShardRequest<>(request, replica.allocationId().getId());
    ...
    protected void sendReplicaRequest(ConcreteShardRequest<ReplicaRequest> concreteShardRequest, DiscoveryNode node, ActionListener<ReplicationOperation.ReplicaResponse> listener) {
        transportService.sendRequest(node, transportReplicaAction, concreteShardRequest, transportOptions, new ActionListenerResponseHandler<ReplicaResponse>(listener, ReplicaResponse::new));
    }
