...
public class MlPlugin extends Plugin implements ActionPlugin {
    ...
    public static final Setting<Boolean> USE_NATIVE_PROCESS_OPTION = Setting.boolSetting("useNativeProcess", true, Property.NodeScope, Property.Deprecated);
    ...
    public Collection<Object> createComponents(Client client, ClusterService clusterService, ThreadPool threadPool, ResourceWatcherService resourceWatcherService, ScriptService scriptService, NamedXContentRegistry xContentRegistry) {
        ...
        if (USE_NATIVE_PROCESS_OPTION.get(settings)) {
            ...
            autodetectProcessFactory = (jobDetails, modelSnapshot, quantiles, filters, ignoreDowntime, executorService) -> new BlackHoleAutodetectProcess();
            normalizerProcessFactory = (jobId, quantilesState, bucketSpan, perPartitionNormalization, executorService) -> new MultiplyingNormalizerProcess(settings, 1.0);
        }
        NormalizerFactory normalizerFactory = new NormalizerFactory(normalizerProcessFactory, threadPool.executor(MlPlugin.THREAD_POOL_NAME));
        AutodetectProcessManager dataProcessor = new AutodetectProcessManager(settings, client, threadPool, jobManager, jobProvider, jobResultsPersister, jobDataCountsPersister, autodetectProcessFactory, normalizerFactory);
        DatafeedJobRunner datafeedJobRunner = new DatafeedJobRunner(threadPool, client, clusterService, jobProvider, System::currentTimeMillis);
        ...
        return Arrays.asList(jobProvider, jobManager, dataProcessor, new MlInitializationService(settings, threadPool, clusterService, jobProvider), jobDataCountsPersister, datafeedJobRunner, persistentActionService, persistentActionRegistry, new PersistentTaskClusterService(Settings.EMPTY, persistentActionRegistry, clusterService));
    }
    ...
    public List<ExecutorBuilder<?>> getExecutorBuilders(Settings settings) {
        ...
        FixedExecutorBuilder ml = new FixedExecutorBuilder(settings, THREAD_POOL_NAME, maxNumberOfJobs * 2, 1000, "xpack.ml.thread_pool");
        FixedExecutorBuilder autoDetect = new FixedExecutorBuilder(settings, AUTODETECT_PROCESS_THREAD_POOL_NAME, maxNumberOfJobs * 4, 4, "xpack.ml.autodetect_process_thread_pool");
        FixedExecutorBuilder datafeed = new FixedExecutorBuilder(settings, DATAFEED_RUNNER_THREAD_POOL_NAME, maxNumberOfJobs, 1, "xpack.ml.datafeed_thread_pool");
