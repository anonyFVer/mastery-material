...
public class IndexAuditTrailTests extends SecurityIntegTestCase {
    ...
    public void initializeRemoteClusterIfNecessary() throws Exception {
        ...
        SecuritySettingsSource cluster2SettingsSource = new SecuritySettingsSource(numNodes, useSSL, createTempDir(), Scope.SUITE) {

            @Override
            public Settings nodeSettings(int nodeOrdinal) {
                Settings.Builder builder = Settings.builder().put(super.nodeSettings(nodeOrdinal)).put("xpack.security.audit.index.settings.index.number_of_shards", numShards).put("xpack.security.audit.index.settings.index.number_of_replicas", numReplicas).put(MachineLearning.AUTODETECT_PROCESS.getKey(), false).put(XPackSettings.SECURITY_ENABLED.getKey(), useSecurity);
        ...
        remoteCluster = new InternalTestCluster(randomLong(), createTempDir(), false, true, numNodes, numNodes, cluster2Name, cluster2SettingsSource, 0, false, SECOND_CLUSTER_NODE_PREFIX, mockPlugins, useSecurity ? getClientWrapper() : Function.identity());
    ...
    public void testAccessDenied() throws Exception {
        ...
        Set<String> specificIndices = randomBoolean() ? randomSpecificIndices() : null;
        String role = randomAlphaOfLengthBetween(1, 6);
        auditor.accessDenied(user, "_action", message, new String[] { role }, specificIndices);
        SearchHit hit = getIndexedAuditMessage(enqueuedMessage.get());
        Map<String, Object> sourceMap = hit.getSourceAsMap();
    ...
    public ClusterHealthStatus ensureYellow(String... indices) {
        ...
        ClusterHealthResponse actionGet = getClient().admin().cluster().health(Requests.clusterHealthRequest(indices).waitForNoRelocatingShards(true).waitForYellowStatus().waitForEvents(Priority.LANGUID)).actionGet();
