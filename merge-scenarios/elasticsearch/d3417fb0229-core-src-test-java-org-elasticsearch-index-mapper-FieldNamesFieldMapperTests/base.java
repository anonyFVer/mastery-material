...
public class FieldNamesFieldMapperTests extends ESSingleNodeTestCase {
    ...
    public void testSeesFieldsFromPlugins() throws IOException {
        ...
        DocumentMapperParser parser = new DocumentMapperParser(indexService.getIndexSettings(), mapperService, indexService.getIndexAnalyzers(), indexService.similarityService(), mapperRegistry, indexService::newQueryShardContext);
        String mapping = XContentFactory.jsonBuilder().startObject().startObject("type").endObject().endObject().string();
