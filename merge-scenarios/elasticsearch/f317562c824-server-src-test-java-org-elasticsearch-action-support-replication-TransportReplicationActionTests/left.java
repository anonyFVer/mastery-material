...
public class TransportReplicationActionTests extends ESTestCase {
    ...
    public void testReplicaProxy() throws InterruptedException, ExecutionException {
        ...
        proxy.performOn(TestShardRouting.newShardRouting(shardId, "NOT THERE", routingState == ShardRoutingState.RELOCATING ? state.nodes().iterator().next().getId() : null, false, routingState), new Request(), randomNonNegativeLong(), listener);
        assertTrue(listener.isDone());
    ...
    public void testReplicasCounter() throws Exception {
        ...
        try {
            replicaOperationTransportHandler.messageReceived(new TransportReplicationAction.ConcreteReplicaRequest<>(new Request().setShardId(shardId), replicaRouting.allocationId().getId(), randomNonNegativeLong(), randomNonNegativeLong()), createTransportChannel(new PlainActionFuture<>()), task);
    ...
    public void testReplicaActionRejectsWrongAid() throws Exception {
        ...
        PlainActionFuture<TestResponse> listener = new PlainActionFuture<>();
        Request request = new Request(shardId).timeout("1ms");
        action.new ReplicaOperationTransportHandler().messageReceived(new TransportReplicationAction.ConcreteReplicaRequest<>(request, "_not_a_valid_aid_", randomNonNegativeLong(), randomNonNegativeLong()), createTransportChannel(listener), maybeTask());
        try {
            listener.get();
            fail("using a wrong aid didn't fail the operation");
        } catch (ExecutionException execException) {
    ...
    public void testRetryOnReplica() throws Exception {
        ...
        replicaOperationTransportHandler.messageReceived(new TransportReplicationAction.ConcreteReplicaRequest<>(request, replica.allocationId().getId(), primaryTerm, checkpoint), createTransportChannel(listener), task);
        if (listener.isDone()) {
            listener.get();
