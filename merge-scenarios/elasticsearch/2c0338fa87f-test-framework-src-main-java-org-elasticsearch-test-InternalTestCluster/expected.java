...
public final class InternalTestCluster extends TestCluster {
    ...
    private synchronized void reset(boolean wipeData) throws IOException {
        ...
        for (int i = 0; i < numSharedDedicatedMasterNodes; i++) {
            ...
            if (autoManageMinMasterNodes) {
                settings.put(INITIAL_STATE_TIMEOUT_SETTING.getKey(), "0s");
            }
                ...
                for (IndexService indexService : indexServices) {
                    for (IndexShard indexShard : indexService) {
                        ...
                        if (activeOperationsCount > 0) {
                            ...
                            try {
                                builder = XContentFactory.jsonBuilder().prettyPrint().startObject().value(response).endObject();
    ...
    private int updateMinMasterNodes(int eligibleMasterNodeCount) {
        ...
        if (getMasterNodesCount() > 0) {
            ...
            try {
                assertAcked(client().admin().cluster().prepareUpdateSettings().setTransientSettings(Settings.builder().put(ElectMasterService.DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING.getKey(), minMasterNodes)));
            } catch (Exception e) {
