...
public class DubboServiceProviderBootstrap {
    ...
    private static void multipleRegistries() {
        ...
        new DubboBootstrap().application(applicationConfig).registries(Arrays.asList(interfaceRegistry, serviceRegistry)).protocol(builder -> builder.port(-1).name("dubbo")).metadataReport(new MetadataReportConfig("zookeeper://127.0.0.1:2181")).service(echoService).service(userService).start().await();
    }
