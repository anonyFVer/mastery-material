...
public class RegistryDirectoryTest {
    ...
    private void testforbid(RegistryDirectory registryDirectory) {
        ...
        registryDirectory.notify(serviceUrls);
        Assertions.assertEquals(false, registryDirectory.isAvailable(), "invokers size=0 ,then the registry directory is not available");
        try {
    ...
    public void testDubbo1UrlWithGenericInvocation() {
        ...
        serviceUrls.add(serviceURL);
        registryDirectory.notify(serviceUrls);
        invocation = new RpcInvocation($INVOKE, new Class[] { String.class, String[].class, Object[].class }, new Object[] { "getXXX1", "", new Object[] {} });
        List<Invoker> invokers = registryDirectory.list(invocation);
        Assertions.assertEquals(1, invokers.size());
    }
    ...
    public void testParmeterRoute() {
        ...
        registryDirectory.notify(serviceUrls);
        invocation = new RpcInvocation($INVOKE, new Class[] { String.class, String[].class, Object[].class }, new Object[] { "getXXX1", new String[] { "Enum" }, new Object[] { Param.MORGAN } });
        List invokers = registryDirectory.list(invocation);
        Assertions.assertEquals(1, invokers.size());
    }
    ...
    public void testNotifyRouterUrls_Clean() {
        if (isScriptUnsupported)
            return;
        RegistryDirectory registryDirectory = getRegistryDirectory();
        URL routerurl = URL.valueOf(ROUTE_PROTOCOL + "://127.0.0.1:9096/").addParameter(ROUTER_KEY, "javascript").addParameter(RULE_KEY, "function test1(){}").addParameter(ROUTER_KEY, "script");
        List<URL> serviceUrls = new ArrayList<URL>();
