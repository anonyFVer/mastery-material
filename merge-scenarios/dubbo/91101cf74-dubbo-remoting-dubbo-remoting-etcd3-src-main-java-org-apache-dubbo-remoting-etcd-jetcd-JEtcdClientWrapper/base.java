...
public class JEtcdClientWrapper {
    ...
    private Client prepareClient(URL url) {
        ...
        ClientBuilder clientBuilder = Client.builder().loadBalancerFactory(RoundRobinLoadBalancerFactory.getInstance()).endpoints(endPoints(url.getBackupAddress())).maxInboundMessageSize(maxInboudSize);
    ...
    public List<String> getChildren(String path) {
        try {
            return RetryLoops.invokeWithRetry(new Callable<List<String>>() {
    ...
    public long createLease(long second) {
        try {
            return RetryLoops.invokeWithRetry(new Callable<Long>() {
    ...
    public void revokeLease(long lease) {
        try {
            RetryLoops.invokeWithRetry(new Callable<Void>() {
    ...
    public boolean checkExists(String path) {
        try {
            return RetryLoops.invokeWithRetry(new Callable<Boolean>() {
    ...
    public String[] endPoints(String backupAddress) {
        ...
        List<String> addressess = Arrays.stream(endpoints).map(address -> address.indexOf(Constants.HTTP_SUBFIX_KEY) > -1 ? address : Constants.HTTP_KEY + address).collect(toList());
