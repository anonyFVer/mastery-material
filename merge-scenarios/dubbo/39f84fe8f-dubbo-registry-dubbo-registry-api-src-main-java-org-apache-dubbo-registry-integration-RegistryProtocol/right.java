...
public class RegistryProtocol implements Protocol {
    ...
    public <T> void reExport(final Invoker<T> originInvoker, URL newInvokerUrl) {
        ...
        getRegisteredUrl(registryUrl, newProviderUrl).ifPresent(oldProviderUrl -> {
    ...
    private Optional<URL> getRegisteredUrl(URL registryUrl, URL providerUrl) {
        ...
        Optional<ProviderModel.RegisterStatedURL> statedUrlOptional = statedUrls.stream().filter(u -> u.getRegistryUrl().equals(registryUrl) && u.getProviderUrl().getProtocol().equals(providerUrl.getProtocol())).findFirst();
    ...
    protected URL getRegistryUrl(URL url) {
        return URLBuilder.from(url).setProtocol(url.getParameter(REGISTRY_KEY, DEFAULT_REGISTRY)).removeParameter(REGISTRY_KEY).build();
    }

    private URL getUrlToRegistry(final URL providerUrl, final URL registryUrl) {
        if (!registryUrl.getParameter(SIMPLIFIED_KEY, false)) {
            return providerUrl.removeParameters(getFilteredKeys(providerUrl)).removeParameters(MONITOR_KEY, BIND_IP_KEY, BIND_PORT_KEY, QOS_ENABLE, QOS_HOST, QOS_PORT, ACCEPT_FOREIGN_IP, VALIDATION_KEY, INTERFACES);
        } else {
    ...
    public void destroy() {
        ...
        bounds.clear();
        ExtensionLoader.getExtensionLoader(GovernanceRuleRepository.class).getDefaultExtension().removeListener(ApplicationModel.getApplication() + CONFIGURATORS_SUFFIX, providerConfigurationListener);
    }
    ...
    private class ExporterChangeableWrapper<T> implements Exporter<T> {
        ...
        public void unexport() {
            ...
            try {
                ...
                ExtensionLoader.getExtensionLoader(GovernanceRuleRepository.class).getDefaultExtension().removeListener(subscribeUrl.getServiceKey() + CONFIGURATORS_SUFFIX, serviceConfigurationListeners.get(subscribeUrl.getServiceKey()));
            } catch (Throwable t) {
                logger.warn(t.getMessage(), t);
            }
            executor.submit(() -> {
