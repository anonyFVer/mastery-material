    ...
    public DubboBootstrap start() {
        if (!isStarted()) {
            ...
            if (!onlyRegisterProvider && !configManager.getServiceConfigs().isEmpty()) {
                List<URL> exportedURLs = exportMetadataService(configManager.getApplication().orElseThrow(() -> new IllegalStateException("ApplicationConfig cannot be null")), configManager.getRegistries(), configManager.getProtocols());
    ...
    private DynamicConfiguration prepareEnvironment(ConfigCenterConfig configCenter) {
        if (configCenter.isValid()) {
            ...
            if (isNotEmpty(appGroup)) {
                appConfigContent = dynamicConfiguration.getConfigs(isNotEmpty(configCenter.getAppConfigFile()) ? configCenter.getAppConfigFile() : configCenter.getConfigFile(), appGroup);
            }
            try {
