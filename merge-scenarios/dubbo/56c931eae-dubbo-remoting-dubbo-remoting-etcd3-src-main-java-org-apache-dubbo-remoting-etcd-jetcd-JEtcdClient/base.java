...
public class JEtcdClient extends AbstractEtcdClient<JEtcdClient.EtcdWatcher> {
    ...
    public JEtcdClient(URL url) {
        ...
        try {
            ...
            reconnectSchedule = Executors.newScheduledThreadPool(1, new NamedThreadFactory("etcd3-watch-auto-reconnect"));
            notifyExecutor = new ThreadPoolExecutor(1, url.getParameter(ETCD3_NOTIFY_MAXTHREADS_KEYS, DEFAULT_ETCD3_NOTIFY_THREADS), DEFAULT_SESSION_TIMEOUT, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(url.getParameter(DEFAULT_ETCD3_NOTIFY_QUEUES_KEY, DEFAULT_GRPC_QUEUES * 3)), new NamedThreadFactory("etcd3-notify", true));
            clientWrapper.start();
        } catch (Exception e) {
    ...
    public class EtcdWatcher implements StreamObserver<WatchResponse> {
            ...
            if (listener != null) {
                ...
                while (iterator.hasNext()) {
                    ...
                    switch(event.getType()) {
                        case PUT:
                            {
                                if (((service = find(event)) != null) && safeUpdate(service, true)) {
                                    modified++;
                                }
                                break;
                            }
                        case DELETE:
                            {
                                if (((service = find(event)) != null) && safeUpdate(service, false)) {
                                    modified++;
                                }
                                break;
                            }
        ...
        private List<String> filterChildren(List<String> children) {
            ...
            return children.stream().parallel().filter(child -> {
                ...
                return count == 1;
            }).map(child -> child.substring(len + 1)).collect(toList());
        }

        protected WatchRequest nextRequest() {
            WatchCreateRequest.Builder builder = WatchCreateRequest.newBuilder().setKey(ByteString.copyFromUtf8(path)).setRangeEnd(ByteString.copyFrom(OptionUtil.prefixEndOf(ByteSequence.from(path, UTF_8)).getBytes())).setProgressNotify(true);
