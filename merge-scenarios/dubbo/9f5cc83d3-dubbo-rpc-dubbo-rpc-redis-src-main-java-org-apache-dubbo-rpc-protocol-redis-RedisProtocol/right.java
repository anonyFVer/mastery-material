...
public class RedisProtocol extends AbstractProtocol {
    ...
    protected <T> Invoker<T> doRefer(final Class<T> type, final URL url) throws RpcException {
        try {
            ...
            final JedisPool jedisPool = new JedisPool(config, url.getHost(), url.getPort(DEFAULT_PORT), url.getParameter(Constants.TIMEOUT_KEY, Constants.DEFAULT_TIMEOUT), StringUtils.isBlank(url.getPassword()) ? null : url.getPassword(), url.getParameter("db.index", 0));
