...
public class PieChartRenderer extends DataRenderer {
        ...
        for (int j = 0; j < dataSet.getEntryCount(); j++) {
            ...
            if ((Math.abs(e.getVal()) > 0.000001)) {
                if (!mChart.needsHighlight(e.getXIndex(), mChart.getData().getIndexOfDataSet(dataSet))) {
                    ...
                    mBitmapCanvas.drawArc(mChart.getCircleBox(), rotationAngle + (angle + sliceSpace / 2f) * mAnimator.getPhaseY(), (newangle - sliceSpace / 2f) * mAnimator.getPhaseY(), true, mRenderPaint);
                }
            }
    ...
    public void drawValues(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = 0, maxEntry = Math.min((int) Math.ceil(entryCount * mAnimator.getPhaseX()), entryCount); j < maxEntry; j++) {
                ...
                float x = (float) (r * Math.cos(Math.toRadians(rotationAngle) + Math.toRadians(angle)) + center.x);
                float y = (float) (r * Math.sin(Math.toRadians(rotationAngle) + Math.toRadians(angle)) + center.y);
