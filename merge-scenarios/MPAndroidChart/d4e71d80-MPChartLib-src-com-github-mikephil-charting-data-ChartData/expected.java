...
public abstract class ChartData<T extends IDataSet<? extends Entry>> {
    ...
    public Entry getEntryForHighlight(Highlight highlight) {
        if (highlight.getDataSetIndex() >= mDataSets.size())
            ...
            List<?> entries = mDataSets.get(highlight.getDataSetIndex()).getEntriesForXIndex(highlight.getXIndex());
            for (Object entry : entries) if (((Entry) entry).getY() == highlight.getValue() || Float.isNaN(highlight.getValue()))
                return (Entry) entry;
            return null;
        }
    }
