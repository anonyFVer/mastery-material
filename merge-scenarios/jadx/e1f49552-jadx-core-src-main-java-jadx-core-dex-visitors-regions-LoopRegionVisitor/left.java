...
@JadxVisitor(name = "LoopRegionVisitor", desc = "Convert 'while' loops to 'for' loops (indexed or for-each)", runBefore = { ProcessVariables.class })
public class LoopRegionVisitor extends AbstractVisitor implements IRegionVisitor {
    ...
    private static boolean checkForIndexedLoop(MethodNode mth, LoopRegion loopRegion, IfCondition condition) {
        ...
        if (phiInsn == null || phiInsn.getArgsCount() != 2 || !phiInsn.containsArg(incrArg) || incrArg.getSVar().getUseCount() != 1) {
            return false;
        }
    ...
    private static boolean checkIterableForEach(MethodNode mth, LoopRegion loopRegion, IfCondition condition) {
        ...
        if (!checkInvoke(hasNextCall, "java.util.Iterator", "hasNext()Z", 0) || !checkInvoke(nextCall, "java.util.Iterator", "next()Ljava/lang/Object;", 0)) {
            return false;
        }
    ...
    private static boolean fixIterableType(MethodNode mth, InsnArg iterableArg, RegisterArg iterVar) {
        ...
        if (iterableType.isGeneric()) {
            ...
            if (wildcardType != null && gType.getWildcardBounds() == 1 && ArgType.isInstanceOf(mth.root(), wildcardType, varType)) {
                return true;
            }
