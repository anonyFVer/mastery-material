...
public class SSATransform extends AbstractVisitor {
        ...
        for (InsnNode insn : block.getInstructions()) {
            if (insn.getType() != InsnType.PHI) {
                for (InsnArg arg : insn.getArguments()) {
                    ...
                    if (var == null) {
                        throw new JadxRuntimeException("Not initialized variable reg: " + regNum + ", insn: " + insn + ", block:" + block);
                    }
                    var.use(reg);
                }
            }
    ...
    private static void markOneArgAsThis(RegisterArg arg) {
        ...
        if (parentInsn != null && parentInsn.getType() == InsnType.MOVE && parentInsn.getArg(0) == arg) {
            RegisterArg resArg = parentInsn.getResult();
            if (resArg.getRegNum() != arg.getRegNum() && !resArg.getSVar().isUsedInPhi()) {
                markThisArgs(resArg);
