...
public class MethodNode extends LineAttrNode implements ILoadable, IDexNode {
    ...
    public boolean isDefaultConstructor() {
        ...
        if (accFlags.isConstructor() && mthInfo.isConstructor()) {
            ...
            if (parentClass.getClassInfo().isInner() && !parentClass.getAccessFlags().isStatic()) {
                ClassNode outerCls = parentClass.getParentClass();
                if (argsList != null && !argsList.isEmpty() && argsList.get(0).getInitType().equals(outerCls.getClassInfo().getType())) {
                    defaultArgCount = 1;
                }
            }
    ...
    public String toString() {
        return parentClass + "." + mthInfo.getName() + "(" + Utils.listToString(mthInfo.getArgumentsTypes()) + "):" + retType;
    }
}