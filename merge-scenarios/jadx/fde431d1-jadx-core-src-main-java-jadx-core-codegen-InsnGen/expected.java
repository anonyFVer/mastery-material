...
public class InsnGen {
    ...
    private static enum IGState {
        ...
        SKIP, NO_SEMICOLON, NO_RESULT, BODY_ONLY, BODY_ONLY_NOWRAP
    }
    ...
    private void makeInsnBody(CodeWriter code, InsnNode insn, EnumSet<IGState> state) throws CodegenException {
        switch(insn.getType()) {
            ...
            case NEW_ARRAY:
                {
                    ...
                    for (int i = 0; i < dim - 1; i++) code.add("[]");
                    break;
                }
            case ARRAY_LENGTH:
            ...
            case IF:
                ...
                String cond = arg(insn.getArg(0)) + " " + ifInsn.getOp().getSymbol() + " " + (ifInsn.isZeroCmp() ? "0" : arg(insn.getArg(1)));
            ...
            case SWITCH:
                ...
                for (int i = 0; i < sw.getCasesCount(); i++) {
                    code.startLine("case " + sw.getKeys()[i] + ": goto " + MethodGen.getLabelName(sw.getTargets()[i]) + ";");
                }
    ...
    private void fillArray(FillArrayNode insn, CodeWriter code) throws CodegenException {
        ...
        if (!elType.equals(insnElementType) && !insnArrayType.equals(ArgType.OBJECT)) {
            ErrorsCounter.methodError(mth, "Incorrect type for fill-array insn " + InsnUtils.formatOffset(insn.getOffset()));
        }
    ...
    private void makeConstructor(ConstructorInsn insn, CodeWriter code, EnumSet<IGState> state) throws CodegenException {
        ClassNode cls = root.resolveClass(insn.getClassType());
        if (cls != null && cls.isAnonymous()) {
            ClassInfo parent;
            if (cls.getSuperClass() != null && !cls.getSuperClass().getFullName().equals("java.lang.Object"))
    ...
    private void makeInvoke(InvokeNode insn, CodeWriter code) throws CodegenException {
        ...
        if (callMthNode != null && callMthNode.getAttributes().contains(AttributeType.METHOD_INLINE)) {
