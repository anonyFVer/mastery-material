    ...
    private static InsnNode simplifyArith(ArithNode arith) {
        ...
        if (litArg != null) {
            ...
            if (arith.getOp() == ArithOp.ADD && lit < 0) {
                return new ArithNode(ArithOp.SUB, arith.getResult(), arith.getArg(0), InsnArg.lit(-lit, litArg.getType()));
            }
            InsnArg firstArg = arith.getArg(0);
            if (arith.getOp() == ArithOp.XOR && firstArg.getType() == ArgType.BOOLEAN && (lit == 0 || lit == 1)) {
