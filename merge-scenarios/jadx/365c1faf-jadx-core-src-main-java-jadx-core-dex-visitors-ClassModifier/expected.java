...
@JadxVisitor(name = "ClassModifier", desc = "Remove synthetic classes, methods and fields", runAfter = { ModVisitor.class, FixAccessModifiers.class })
public class ClassModifier extends AbstractVisitor {
        ...
        if (inline || cls.getClassInfo().isInner()) {
            for (FieldNode field : cls.getFields()) {
                if (field.getAccessFlags().isSynthetic() && field.getType().isObject()) {
                    ...
                    if (fieldsCls != null && (inline || parentClass.equals(fieldsCls.getClassInfo()))) {
                        int found = 0;
    ...
    private static boolean isRemovedClassInArgs(ClassNode cls, List<RegisterArg> mthArgs) {
        for (RegisterArg arg : mthArgs) {
            ...
            if (argCls == null) {
                ...
                if (argClsInfo.isInner() && cls.getFullName().startsWith(argClsInfo.getParentClass().getFullName())) {
                    return true;
                }
            } else {
    ...
    private static boolean isMethodUnique(ClassNode cls, MethodNode mth) {
        ...
        for (MethodNode otherMth : cls.getMethods()) {
            if (otherMth != mth) {
                ...
                if (omi.getName().equals(mi.getName()) && Objects.equals(omi.getArgumentsTypes(), mi.getArgumentsTypes())) {
                    return false;
                }
            }
        }
    ...
    private static void removeEmptyMethods(MethodNode mth) {
        AccessInfo af = mth.getAccessFlags();
        if (af.isConstructor() && (af.isPublic() || af.isStatic()) && mth.getArguments(false).isEmpty()) {
    ...
    private static boolean isNonDefaultConstructorExists(MethodNode defCtor) {
        ...
        for (MethodNode mth : parentClass.getMethods()) {
            if (mth != defCtor && mth.getAccessFlags().isConstructor() && mth.getMethodInfo().isConstructor() && !mth.isDefaultConstructor()) {
                return true;
            }
        }
        return false;
